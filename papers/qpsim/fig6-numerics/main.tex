...
isolated with dedicated controls.  Grid refinement to $\mathrm{d}E = 0.25\ueV$ shows the
center-carrier reading already converged at the published resolution
while the left-edge reading still moves by $\sim 9\%$ per halving ---
the published observable carries a discretization offset comparable to
the inter-code discrepancy itself.  We argue that figures constructed
...
identical states.  \emph{Second}, an attribution: the entire historical
...
one exact full-resolution point replayed \\
...
open circles: the published numerical curves digitized from the
arXiv raster (error bars smaller than the markers).  The published
points lie \emph{between} the two readings of one and the same
solution family; $T_B=0.10$~K is excluded per
Sec.~\ref{sec:certificates}.  Diagnostic artifact:
...
\colrule
author sampling & 0.1454 & 0.1326 & 0.1262 & 0.1220 & \todo{de8} \\
center sampling & 0.0505 & 0.0480 & 0.0477 & 0.0479 & \todo{de8} \\
\end{tabular}
\end{ruledtabular}
\end{table}

\noindent
The center-carrier reading is grid-converged at the published
resolution (stable within $\pm 0.0002$ from $\mathrm{d}E = 0.5\ueV$
on), whereas the left-edge reading of the same solved states is still
moving at $\mathrm{d}E = 0.25\ueV$, with successive differences
decaying \emph{sub-linearly} (apparent order $\sim 0.3$, consistent
with an edge-cell-dominated quadrature error).  Both readings are
consistent discretizations of the same continuum functional and must
meet as $\mathrm{d}E \to 0$; the measured sequence already shows that
the published observable, at the published $1\ueV$ resolution and under
its own sampling convention, carries a discretization offset of at
least $\sim 20\%$ relative to its refined value --- comparable to the
entire inter-code discrepancy that motivated this study.  We
deliberately refrain from a three-parameter continuum extrapolation:
the four points do not determine the limit robustly, and the slow
left-edge decay merits its own analytic treatment of the singular
gap-edge cells. \todo{physicist review; fill the
$\mathrm{d}E = 0.125\ueV$ column when the run completes.}
