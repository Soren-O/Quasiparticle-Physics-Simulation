...
% ---------------- the fork (same average, distinct first-harmonic closures) ----------------
\node[font=\scriptsize, text=slate, align=center] at (0,-3.30)
  {$\langle R_sX\rangle_{\hat{\mathbf p}}
    =\langle X\rangle_{\hat{\mathbf k}}$,\quad
    $\hat{\mathbf k}=s\hat{\mathbf p}$\\[-1pt]
   (bare average commutes; first-harmonic closure differs)};
...
\draw[arr] (eilen.south west)
  -- node[lab, pos=0.60, left=3pt]
     {matrix moment closure\\(dirty, $\ell\ll\xi_0$)} (usadel.north);
\draw[redarr] (eilen.south east)
  -- node[lab, pos=0.60, right=3pt]
     {branch projection,\\then scalar moments} (branch.north);
