...
identities are used (\cref{sec:usadel_route}).
...
\(\DOS^2-N_2^2=1\),\footnote{This identity is where lifetime broadening
matters algebraically: inserting a finite \(\ii\Gamma\) directly into the
spectral functions changes the clean BCS identity \(\DOS^2-N_2^2=1\) used
here---the matrix normalization \(g^2-f^2=1\) survives the complex shift
\(E\to E+\ii\Gamma\); it is the identity among the \emph{real} spectral
coefficients that fails---and therefore changes these trace
simplifications rather than merely smoothing the plotted coefficient.
Where a finite \(\Gamma\) is physically required, the Nambu trace
identities of SM~\cref{SM-app:derivation} must be re-evaluated ---
generally numerically, since the real-BCS simplification no longer
applies --- with the complex spectral functions \(\DOS(E+\ii\Gamma)\),
\(N_2(E+\ii\Gamma)\); this also regularizes the gap-edge structure of the
spectral coefficients (e.g.\ the divergence of the transverse dressing
