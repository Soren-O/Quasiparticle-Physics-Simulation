...
charge imbalance, we show that two microscopic formalisms, a
branch-projected scalar kinetic formalism and a Keldysh--Usadel
formalism, are related by an exact change of variables. Up to
collision and spectral-flow terms these yield the same diffusion equation:
...
deviate from his.
We start by defining the \(\fL\) and \(\fT\) quasiparticle distribution amplitudes.
...
density of states at the Fermi level. It is equivalent to Kopnin's
...
\(E>\Delta\), and before any angular averaging.
...
rewriting below (\cref{eq:intro_boltzmann_clean}) the same adiabatic
...
\cref{eq:beta_twomode_L}---the ``spatial current.''
Both routes are organized around the \emph{angular
harmonics} of the distribution amplitudes, their dependence on the
trajectory direction \(\hat{\bp}\) over the Fermi surface. Explicitly,
at fixed \((\br,E,t)\) each amplitude is a function on the unit sphere
of trajectory directions and can be expanded in spherical harmonics
\(Y_{\ell m}(\hat{\bp})\). The three \(\ell=1\) harmonics span the
components of \(\hat{\bp}\) itself, so for a generic amplitude \(X\)
the expansion through the dipole reads
\begin{equation}
  X(E,\hat{\bp},\br,t)
  =
  X_{0}(E,\br,t)
  +
  \hat{\bp}\cdot\mathbf{X}_{1}(E,\br,t)
  +
  \cdots,
  \qquad
  X_{0}
  =
  \avg{X}_{\hat{\bp}},
  \label{eq:beta_harmonic_expansion}
\end{equation}
with the Cartesian vector \(\mathbf{X}_{1}\) collecting the three
dipole coefficients and the ellipsis standing for the \(\ell\ge2\)
harmonics. The isotropic
part (\(\ell=0\)) of \(\fL\) carries the energy distribution and sources
the \(s\)-wave gap; the dipole (\(\ell=1\)) of \(\fT\) carries the
spatial current. This is why the
current flows through the transverse amplitude \(\fT\) (the
\(\bnabla_{\br}(\DOS\,\fT)\) term, not \(\fL\) directly):
\cref{eq:beta_twomode_T} slaves the anisotropic (dipole) harmonic of
\(\fT\) to \(\bnabla_{\br}\fL\), and the dirty route isotropizes this
slaved structure into diffusion. A finer classification, by parity of the
harmonic under the inversion \(\hat{\bp}\to-\hat{\bp}\) (even vs.\ odd),
will matter once we relabel the branches below; because we truncate the
dirty-limit expansion at the dipole (\cref{sec:sc_angular_averaging}), the
two cuts coincide there, with the isotropic mode even and the
current-carrying dipole odd. We carry this even/odd distinction explicitly
from here on. The clean route instead recombines the two
modes without eliminating any angular harmonic and may retain
\(\mathcal{R}[\fL]\) at augmented-clean
quasiclassical order; retaining the force is legitimate while
\(|\Delta|\) and any supercurrent are nearly uniform. It is finally
dropped at standard Eilenberger order (next subsection).


\subsection{Quasiparticle Boltzmann-Like Equations}
The coupled two-mode pair \cref{eq:beta_twomode_L,eq:beta_twomode_T} is
not the only way to carry this content. We now recast it as a
\emph{single} scalar Boltzmann-like equation for the occupation of
positive-energy quasiparticles via an exact change of basis. The two
amplitudes \(\fL\) and \(\fT\) are repackaged as the two parity components
of one branch occupation. 
Concretely, we change variables from the mode \textit{amplitudes},
\(\fL \text{ and }\fT\), to the \textit{occupations} of the two quasiparticle
branches (the electron-like (\(\xi_{\bk}>0\)) and hole-like
(\(\xi_{\bk}<0\)) halves of the spectrum). We label the
positive-energy occupation \(f_{\bk}\), or equivalently the distribution
amplitude \(h_{\bk}\equiv1-2f_{\bk}\). We keep both because each is natural
on one side of the bridge: \(f_{\bk}\) is the occupation the semiclassical
Boltzmann equation transports and that carries the Pauli factors in the
collision integral, whereas \(h_{\bk}\) is the amplitude that reconnects to
the two-mode description, from which \(\fL\) and \(\fT\) are recovered, and
its equilibrium form is \(h^{(0)}=\tanh(E/2k_{\mathrm B}T)=\fL^{(0)}\). The
two are related by the affine map \(h_{\bk}=1-2f_{\bk}\), so the choice
between them is one of convenience. If fields are present it is the gauge-invariant transverse mode that should be used,
\begin{equation}
...
\end{equation}
with \(\varphi\) the scalar potential, \(\chi\) the order-parameter
phase, and \(\bA\) the vector potential.
...
\end{equation}
We define the branch label \( \sigma \) and the BCS velocity factor \( \lambda_{\bk} \):
\begin{equation}
...
The explicit change of variables is given by
\begin{equation}
  h_{\bk}
...
\end{equation}
with all amplitudes
evaluated at \((\varepsilon_{\bk},\hat{\bk},\br,t)\). In the
quasiparticle-occupation convention used throughout both branches
carry the same occupation \(f(E)\) in
equilibrium.\footnote{This convention is equivalent to Eq.~(15.4) of
Ref.~\onlinecite{kopnin2001} with the hole branch's occupation
conjugated. If \(n_{\bk}^{\mathrm K}\) denotes Kopnin's occupation,
whose equilibrium form is the Fermi function of \(\varepsilon_{\bk}\)
only on the electron-like branch, the conjugation is the single line
\(h_{\bk}=\sigma\bigl(1-2n_{\bk}^{\mathrm K}\bigr)
=f^{(1)}+\sigma\,\lambda_{\bk}^{-1}f_2'\), identifying \(\fL=f^{(1)}\)
and \(\fT'=f_2'\): the branch sign \(\sigma\) multiplies the
longitudinal amplitude in Kopnin's convention and the transverse one
in ours.} 
...
\(h_\sigma\), and it is nonzero in equilibrium with \(\fL^{(0)}=\tanh(E/2k_{\mathrm
B}T)\). The (shifted) transverse mode is the branch-odd part weighted
by \(\lambda_{\bk}\), and it is zero in equilibrium. 

The change of variables so far is purely algebraic; to turn it into a
Boltzmann-like equation we need the equations of motion for the two
branches. The semiclassical motion is \(\dot\br=\bvg\) and
\begin{equation}
  \hbar\dot\bk = \bF_{\bk}
...
\end{equation}
(In \(\dot{\hat{\bk}}\) the radial magnitude has been set
\(k\to k_{\mathrm F}\), to quasiclassical accuracy;
...
Transported by this Liouville operator, the branch occupation \(f_{\bk}\)
obeys the single Boltzmann-like equation
\begin{equation}
...
quasiparticle--quasiparticle), whose elastic (impurity) part is
specified in \cref{eq:intro_impurity_collision} below.

To see that this single equation is equivalent to the two-mode pair we
write it for each branch
\begin{equation}
  \mathcal L_\sigma h_\sigma = -2\,I_{\mathrm{qp},\sigma},
  \label{eq:beta_h_boltzmann}
\end{equation}
and form its two parity projections
\begin{align}
  \mathcal K_L
...
\end{align}
By \cref{eq:beta_fL_inverse,eq:beta_fT_inverse}, and within the local
positive-energy quasiparticle approximation, these projections are the
...
does not relax the isotropic energy mode. It does, however, relax the
anisotropic, current-carrying harmonics, meaning that elastic impurity
scattering randomizes the quasiparticle's direction of motion. This
momentum relaxation turns ballistic streaming into diffusion once
scattering is strong, yet it spares both isotropic modes: it conserves
energy, and it cannot relax the net charge imbalance, whose decay would
require a branch-crossing process of vanishing coherence factor. 
...
We now rewrite \cref{eq:beta_boltzmann_energy_variables} in energy--angle form (the substitution is recorded in
\cref{app:change_of_variables}):
\begin{equation}
...
\end{equation}
Where we have set \(k\to k_{\mathrm F}\) in the \( \dot{\hat{\bk}}\cdot \nabla_{\hat{\bk}} f\) term (a
quasiclassically relative error \(O(|\xi_{\bk}|/E_{\mathrm F})\),
at most \(O(\omega_D/E_{\mathrm F})\) across the pairing shell). Note that the angular (refraction) term is of next order in
\((k_{\mathrm F}\xi_0)^{-1}\) which is beyond the standard Eilenberger equation
\cref{eq:intro_eilenberger}, so we drop it, together with the other
...
Next, we resolve the occupation into its two scalar modes. The change
of variables \(\bk\to(E,\hat{\bk})\) is two-to-one: at each \(E>\Delta\)
and momentum direction \(\hat{\bk}\) the two branches
\(s=\operatorname{sgn}\xi_{\bk}=\pm\) (the \(\Phi\to0\) limit of the
branch sign \(\sigma\) of \cref{eq:beta_sigma_lambda}) stream oppositely, since
...
It is convenient at this point to relabel each branch by its \emph{trajectory} (group-velocity)
direction \(\hat{\bp}=s\,\hat{\bk}\). That is, we set 
\(\widetilde f_s(E,\hat{\bp},\br,t)\equiv f_s(E,s\hat{\bp},\br,t)\) and
likewise \(\widetilde I_s\). This absorbs the sign,
so that each \(\widetilde f_s\) obeys the same equation,
\begin{equation}
  \Bigl[\partial_t+\tfrac{\Delta}{E}\dot{\Delta}\,\partial_E
...
\end{equation}
This equation has a \emph{diagonal drift}: the streaming operator on the
left is identical for both branches, so it carries over unchanged to any
linear combination of them without generating cross terms. The only
mixing between such combinations then comes from the collision integrals
and the self-consistent gap.

The branch-symmetric combination \(\fL=1-\widetilde f_+-\widetilde f_-\)
and the \emph{unweighted} branch-antisymmetric combination
\(\phi_T\equiv\widetilde f_--\widetilde f_+\) have the same
diagonal drift. The transverse amplitude \(\fT\) does not, and the even
and odd angular harmonics behave differently. Recall that \(\fT\) is
\(\phi_T\) scaled by the spectral weight \(\lambda_{\bk}\): for the even
harmonics \(\fT=\lambda_{\bk}\,\phi_T\) (\cref{eq:beta_fT_inverse}; here
\(\fT=\fT'\), since the gauge shift \(\Phi\to0\)). Because of this weight,
\(\fT\) inherits the diagonal drift only up to the spectral-weight
derivative \(D\ln\lambda_{\bk}\) of \cref{eq:intro_fT_kopnin}. The odd
harmonics instead obey the sector swap of
\cref{eq:intro_parity_dictionary}. What keeps the streaming diagonal in
the first place is dropping the refraction force
\(\tfrac{\Delta}{\hbar E k_{\mathrm F}}
\bigl[(\openone-\hat{\bk}\hat{\bk})\bnabla_{\br}\Delta\bigr]\cdot
\bnabla_{\hat{\bk}}f\) of \cref{eq:intro_boltzmann_clean} before the modes
are formed: had it been retained, the relabelling would have rendered it
branch-odd and coupled \(\fL\) to \(\phi_T\).

The collision kernels and any boundary conditions are relabelled
identically. This is invisible for isotropic kernels under Fermi-surface
averaging, but at an interface or under directional injection one must
remember that the trajectory label \(\hat{\bp}\) stands for the physical
momentum direction \(\hat{\bk}=s\hat{\bp}\), to which it is antipodal on
the hole-like branch. From here \(\hat{\bp}\) denotes this trajectory
(group-velocity) direction and the streaming speed is the
branch-independent magnitude \(\vg(E)\).

A final piece of parity bookkeeping protects a step taken repeatedly
below. The applications in this paper work in the charge-balanced limit
\(\phi_T\to0\), and since the transverse amplitude is tied to \(\phi_T\)
by \(\fT=\lambda_{\bk}\phi_T\), that limit might seem to eliminate
\(\fT\) altogether, and with it the \(\bnabla_{\br}(\DOS\,\fT)\) term
through which the spatial current flows. It does not: the identity is a
statement about the even angular harmonics only, and the current
survives in the odd ones. To make this precise we change variables one
final time, from the momentum-direction modes \((\fL,\fT)\) to the
branch-even and branch-odd combinations of the relabelled occupations,
\begin{equation}
  F\equiv\widetilde f_++\widetilde f_-,
  \qquad
  \phi_T\equiv\widetilde f_--\widetilde f_+,
  \label{eq:intro_FphiT_def}
\end{equation}
(note the branch-odd \(\phi_T\) was already introduced above). The
diffusive reduction below operates one angular harmonic at a time
(\cref{sec:sc_angular_averaging}), so we will need the map between these
combinations and the physical modes \((\fL,\fT)\) harmonic by harmonic.
Solving the mode definitions for the new pair, one might guess the
pointwise inverse
\begin{align}
  \phi_T &\overset{?}{=} \lambda_{\bk}^{-1}\,\fT, &
  F &\overset{?}{=} 1-\fL ,
  \label{eq:intro_naive_map}
\end{align}
i.e.\ \(\fT=\lambda_{\bk}\phi_T\) and \(\fL=1-F\) read off directly. This
holds for the even angular harmonics but fails for the odd, because the
change of variables is not pointwise: it ties each direction to its
antipode. To see this, write the branch occupations out. The modes
combine the two branches at
the same momentum direction \(\hat{\bk}\):
\(\fL=1-f_+-f_-\) and \(\fT=\lambda_{\bk}(f_--f_+)\), both evaluated at
\(\hat{\bk}\) [\cref{eq:beta_fL_inverse,eq:beta_fT_inverse}]. The
combinations \(\phi_T\) and \(F\)
instead group them at the same trajectory direction \(\hat{\bp}\), and the
relabelling \(\widetilde f_-(\hat{\bp})=f_-(-\hat{\bp})\) flips the hole
branch's argument: \(\phi_T=f_-(-\hat{\bp})-f_+(\hat{\bp})\) and
\(F=f_+(\hat{\bp})+f_-(-\hat{\bp})\). At a common direction the two
groupings therefore differ only in \(f_-(\hat{\bk})\) versus
\(f_-(-\hat{\bk})\), which is invisible to the even angular harmonics and
a sign to the odd. Splitting each amplitude into parts even and odd under
inversion of its direction argument, the inverse relations
\cref{eq:beta_fL_inverse,eq:beta_fT_inverse} combined with the relabelling
give
\begin{equation}
...
\end{equation}
the odd harmonics swap sectors. This resolves the puzzle that opened the
paragraph: \(\fT=\lambda_{\bk}\phi_T\) is an identity for the even
harmonics only (in particular the isotropic, charge-imbalance harmonic),
so the limit \(\phi_T\to0\) removes the charge-imbalance content of
\(\fT\), while its odd harmonics, built from the branch-\emph{even}
\(F\), survive as the current-carrying channel of the diffusive
reduction (\cref{sec:sc_angular_averaging}). Nor does \(F\) retire after
this paragraph: once \(\phi_T=0\) the two branches share a single
occupation \(f\), so \(F=2f\), and the dipole part of \(F\) is twice the
\(\hat{\bp}\cdot\mathbf{a}\) harmonic retained by the moment expansion
and then eliminated in favor of \(\bnabla_{\br}\fL\) to produce the
diffusive current [\cref{eq:sc_hidden_harmonic}]. The practical rule is
only that the even-harmonic identity must not be carried over to the odd
harmonics. The distribution variables introduced across this change of
basis, and the maps relating them, are collected in
\cref{tab:change_of_variables}.

\begin{table*}[!tbp]
\caption[Distribution variables in the change of variables]{%
\label{tab:change_of_variables}Distribution variables carried through the
change of basis of \cref{eq:beta_f_from_modes}, from the two-mode
amplitudes \((\fL,\fT)\) to the branch occupations and back. All are
functions of energy \(E\), a direction, position \(\br\), and time \(t\);
the direction argument is the momentum direction \(\hat{\bk}\) before the
trajectory relabelling \(\hat{\bp}=s\hat{\bk}\) and the group-velocity
(trajectory) direction \(\hat{\bp}\) after it. Equilibrium values assume a
common branch occupation \(f(E)\); \(\hat h=\fL\openone+\fT\tau_3\) is the
distribution matrix of \cref{eq:intro_h_matrix}. The identity
\(\fT=\lambda_{\bk}\phi_T\) holds for the even angular harmonics only; the
odd harmonics swap sectors as in \cref{eq:intro_parity_dictionary}.}
\begin{ruledtabular}
\begin{tabular}{p{0.09\textwidth}p{0.27\textwidth}p{0.34\textwidth}p{0.16\textwidth}}
Symbol & Meaning & Definition / relation & Equilibrium \\
\hline
\(\fL\) & longitudinal amplitude (Kopnin \(f^{(1)}\)) &
\(\tfrac12\Tr\hat h=1-F\) & \(\tanh(E/2k_{\mathrm B}T)\) \\
\(\fT\) & transverse amplitude (Kopnin \(f^{(2)}\)); charge imbalance &
\(\tfrac12\Tr[\tau_3\hat h]=\lambda_{\bk}\phi_T\) (even harmonics) & \(0\) \\
\(\fT'\) & gauge-invariant transverse amplitude &
\(\fT-e\Phi\,\partial_E\fL^{(0)}\) (\cref{eq:beta_fT_shift}) & \(0\) \\
\(f_s\) & branch occupation, fixed \(\hat{\bk}\), branch
\(s=\operatorname{sgn}\widetilde\xi_{\bk}=\pm\) &
\(\tfrac12\bigl[1-\fL-s\,\lambda_{\bk}^{-1}\fT'\bigr]\) (\cref{eq:beta_f_from_modes}) & \(f(E)\) \\
\(h_s\) & branch amplitude \((h_s=1-2f_s)\) &
\(\fL+s\,\lambda_{\bk}^{-1}\fT'\) (\cref{eq:beta_f_from_modes}) & \(\tanh(E/2k_{\mathrm B}T)\) \\
\(\lambda_{\bk}\) & spectral weight (BCS velocity factor) &
\(|\widetilde\xi_{\bk}|/\widetilde E_{\bk}\) (\cref{eq:beta_sigma_lambda}) & --- \\
\(\widetilde f_s\) & trajectory-relabelled branch occupation &
\(\widetilde f_s(E,\hat{\bp})=f_s(E,s\hat{\bp})\) & \(f(E)\) \\
\(\phi_T\) & unweighted branch-antisymmetric occupation &
\(\widetilde f_--\widetilde f_+\) & \(0\) \\
\(F\) & branch-even occupation combination &
\(\widetilde f_++\widetilde f_-\); \(\fL=1-F\) & \(2f(E)\) \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
creation do not appear. In this gauge the ballistic streaming is diagonal
in \((\fL,\phi_T)\): the two modes are not coupled by the drift in
\cref{eq:intro_fL,eq:intro_fT}. For the even angular harmonics this is
equivalently the transverse mode \(\fT=\lambda_{\bk}\phi_T\), up to the
spectral-weight derivative \(D\ln\lambda_{\bk}\). Their only direct
coupling is through the collision integrals and the
...
leading quasiclassical order comprises the four coupled equations, namely the
...
amplitude, \(\fT=\lambda_{\bk}\phi_T\) [\(\lambda_{\bk}=|\widetilde\xi_{\bk}|/
...
\end{equation}
the spectral-weight term vanishes when the branch-odd mode \(\phi_T\to0\).
This does not remove the odd momentum-direction harmonic of \(\fT\), which
remains the current-carrying harmonic in \cref{eq:sc_hidden_harmonic}.
...
are neglected \cite{kaplan1976,changscalapino1977}. This statement does not
discard the odd, current-carrying harmonic of \(\fT\), which is built from
the branch-even occupation.
...
\cref{eq:intro_parity_dictionary}, the \emph{even} harmonics of the
...
the boundary condition adopted below \cref{eq:sc_three_gap_diffusive}. This resolves
...
impose the zero-flux face explicitly (as the interface benchmark of
SM~\cref{SM-sec:program} does at a material boundary). Finite Dynes broadening is a
