...
\(\bnabla\cdot[(\DN/\DOS)\bnabla f]\), which treats the occupation
itself as the conserved density and acquires a spurious DOS-gradient
drift once the gap is inhomogeneous---as it always is, at some
amplitude, once the occupation feeds back on the gap. We organize the
...
represented through an effective self-energy. The Eilenberger equation is obtained by taking the Wigner-transformed
Dyson equation gradient expanded to first order in the slow
center-of-mass variables, and the difference of the left- and
...
overall factor \(\ii\) that is multiplying the commutator balances the real
streaming term.
...
through \(\delta N=-\nu(0)\!\int\!\langle\DOS\,\fT\rangle\,\dd E\)
[Kopnin's Eqs.~(10.69)--(10.71)], fixing the sign independent of the
branch labelling. When both branches share a common quasiparticle
...
self-energy \(\check\Sigma_{\mathrm{imp}}=(\ii/2\tau)\langle\check
g\rangle_{\hat{\bp}}\) [Kopnin's \S10.4.1], with \(\tau\) the elastic
...
dimensionless pairing constant, and \(\omega_D\) the Debye energy
...
\cite{kaplan1976,changscalapino1977,goldie2013}; \(I_{T}\) also
...
supercurrent, so that nothing sources \(\phi_T\) in
\cref{eq:intro_fL,eq:intro_fT} and any residual imbalance is relaxed by
\(I_{T}\) \cite{tinkham2004}---one can set the branch-odd sector to zero,
\(\phi_T\to0\)---every angular harmonic of the branch-odd mode, and
with it, by the parity dictionary \cref{eq:intro_parity_dictionary},
the \emph{even} harmonics of Kopnin's transverse amplitude
\(\fT=\lambda_{\bk}\phi_T\); the odd harmonics of \(\fT\) are built
from the branch-even occupation and survive, slaved to
\(\bnabla_{\br}\fL\) in the diffusive reduction
(\cref{sec:sc_angular_averaging})---then redefine the remaining
mode as the still angle-resolved occupation probability
\(f(E,\hat{\bp},\br,t)\equiv\tfrac{1}{2}\bigl[1-\fL(E,\hat{\bp},\br,t)\bigr]\)
[\cref{eq:beta_f_from_modes} at \(\fT'=0\)]---so that the scalar collision integral
\(\Icoll[f,n]\equiv-\tfrac12\,I_L[\,1-2f,0,n\,]\) carries the sign and
...
instead consider adding an impurity collision term to the
...
We expect in both cases to end with an energy-resolved scalar equation of the form
\begin{equation}
  \partial_t f(E,\br,t)
...
\(\LD\) is a spatial transport operator. The question then becomes determining the form of this spatial operator and showing if and when the different routes agree on its form. 

This work is relevant because collision terms in recent
superconducting resonator and qubit treatments are usually spatially
homogeneous \cite{goldie2013,catelani2019,fischer2023,fischer2024,marchegiani2025} or zero dimensional. Spatial simulations of
devices---normal-metal and gap-engineered quasiparticle traps in qubits are
the prototypical examples \cite{riwar2016,hosseinkhani2018,riwar2019}---however, require choosing \(\LD\).
...
operator by notation \cite{dynes1978}. \footnote{This matters algebraically: inserting a
finite \(i\Gamma\) directly into the spectral functions changes the clean BCS
identity \(\DOS^2-N_2^2=1\) used below---the matrix normalization
\(g^2-f^2=1\) survives the complex shift \(E\to E+i\Gamma\); it is the
identity among the \emph{real} spectral coefficients that fails---and
therefore changes the trace
simplifications rather than merely smoothing the plotted coefficient. Where a
finite \(\Gamma\) is physically required, the Nambu trace identities of
SM~\cref{SM-app:derivation} must be re-evaluated --- generally numerically,
since the real-BCS simplification no longer applies --- with the complex spectral
functions \(\DOS(E+\ii\Gamma)\), \(N_2(E+\ii\Gamma)\); this also
regularizes the gap-edge structure of the spectral coefficients (e.g.\ the
divergence of the transverse dressing \(\mathcal D_T=\DOS^2\)) and the
matched-gap singularity of the Kupriyanov--Lukichev charge-channel
interface weight.}
...
unit solid angle of \(\hat{\bp}'\) for elastic scattering within the
...
the spatial operator now derived rather than postulated. The derivation
...
density be \(N(0)\DOS f\) with \(N(0)\) the local normal-state density
of states at the Fermi level, that it obey a continuity law, and that
...
\to\nu(0)\,\DOS(E)\,\avg{\,\cdots}_{\hat{\bp}'}\) on the surviving
...
The Eilenberger equation is a trajectory-resolved quasiclassical transport
equation for the matrix Green's function \(\check{g}(E,\hat{\bp},\br,t)\)
...
matrix level gives the Usadel equation \cite{usadel1970},
\begin{equation}
  \hbar\DN\,
  \bnabla_{\br}\cdot
  \left(
    \check{g}\,\bnabla_{\br}\check{g}
  \right)
  +
  \ii\left[
...
ordinary products and the normalization to \(\check{g}^2=1\). The important point is that the
...
level \cite{schmid1980,belzig1999}.
...
\end{equation}
Taking the Nambu traces and using the BCS identity \(\DOS^2-N_2^2=1\),
\begin{equation}
  \mathcal{D}_L(E)
  =
  1,
...
spectral angle, and the scalar mode it dresses. The table is the lookup
...
The branch reductions can now be compared without changing notation by
...
spatial-drift term in an inhomogeneous gap.
...
shell density is \(w(E,\br)f\) with a Fick law for that density,
...
(\cref{eq:projection_average_commutator}), and the operators of
\cref{tab:operator_taxonomy} are their scalar end points.
...
\begin{figure*}[t]
  ...
  commute termwise (\cref{eq:projection_average_commutator}): the
...
(Fick ansatz for \(f\)); the ordered branch reduction gives \((1,-1)\) &
...
device modeling; it shares the uniform-gap rate of A1, which is why the
...
The benchmarks below treat \(\Delta(\br)\) as prescribed. In a device the
...
\(n_{\qp}=4N_0\int\DOS f\,\dd E\). The amplitude is small in devices
(\(x_{\qp}\sim10^{-9}\)--\(10^{-5}\) in qubits, larger under drive), but
the transport operators' \emph{response} to it is singular at the gap
...
\(v=\DN\,q\,\DOS^{\,q-p-1}\partial_x\DOS\), with the sign of \(q\); the
...
first three tests numerically, evolving every operator with the same
one-dimensional Crank--Nicolson discretization (aluminum-strip
parameters, \(\DN=6~\mu\mathrm{m}^2/\mathrm{ns}\),
\(L=100~\mu\mathrm{m}\), reflective ends; the \((1,2)\) diagnostic is
labeled A1P in the legends). The measured points sit on the analytic
...
\begin{figure}[t]
  ...
  and B fall toward the well, the \(q=2\) diagnostics A1P and A2 are
...
negligible charge imbalance---the scalar diffusion problem has a single
microscopically selected answer, and both reduction routes deliver it.
...
proximity-modified spectra, or nonadiabatic dynamics---with the leading
content of each bounded in
SM~\cref{SM-app:supercurrent,SM-app:proximity,SM-app:nonadiabatic}. The symmetric A2
...
The scalar quasiparticle Boltzmann route gives the rate coefficient
\(D_{\mathrm B}(E)=\vg^2\tautr(E)/3\), and its moment reduction fixes the
operator ordering,
\((\vg/3)\bnabla\cdot(\vg\tautr\bnabla f)
=(1/\DOS)\bnabla\cdot(\DOS D_{\mathrm B}\bnabla f)\). In
the branch-resolved, nonmagnetic elastic \(s\)-wave limit, the
Bardeen--Rickayzen--Tewordt coherence-factor cancellation selects a constant
transport mean free path, \(\DOS D_{\mathrm B}=\DN\), and the branch route
lands on the same operator A1 as the dirty-limit projection: the same
conserved density, the same undressed flux, no DOS-gradient drift. The
legacy device-physics coefficient
\(\DN\sqrt{1-(\Delta/E)^2}=\DN/\DOS(E)\) is therefore
microscopically supported in both routes, but its common placement inside
the divergence, \(\bnabla\cdot[(\DN/\DOS)\bnabla f]\), is not: it treats
the occupation as the conserved density and acquires a spurious
DOS-gradient drift for an inhomogeneous gap. The remaining theoretical work is
not to choose freely among these equations, but to state their regimes of
validity and their crossovers: diffusive versus ballistic kinetics, bulk
spectra versus proximity-modified spectra, and bulk diffusion
versus interface or trap boundary conditions.

For device modeling, the energy-resolved operator also fixes how the spatially
resolved \emph{total} quasiparticle density
...
factor \(4\) counting spin and the two branches \(\pm\xi\)), diffuses in
space.
Integrating the energy-resolved A1 equation over \(E\), and assuming a fixed
...
diffusive weight in both microscopic routes. What the operator placement
...
Finally, none of the scalar operators is expected to hold where the spectral
