...
In the dirty limit we expect a diffusion equation for the quasiparticles.  
The ordinary diffusion equation reads \( \partial_t \chi = \bnabla\cdot(D\,\bnabla \chi) \). In the limit where the quasiparticles can be described by a classical scalar occupation \(f(E,\br,t)\), which energy-resolved diffusion operator governs that occupation when the superconducting gap may vary in space and time? Various operators have been used inconsistently in the literature. 
For a homogeneous normal-state material in the dirty limit, with a
slowly varying local BCS gap, nonmagnetic Born disorder, and negligible
charge imbalance, we show that two microscopic constructions, a
branch-projected scalar kinetic theory and a Keldysh--Usadel
reduction, are related by an exact change of variables and, up to
collision and spectral-flow terms, yield the same diffusion equation:
...
energy.  Neither of these is selected by either reduction, and
both produce a spurious density-of-states-gradient drift for smoothly
varying local-BCS gaps.
...
For a spatially uniform gap these differing choices may reduce to the same result, which is partly why the inconsistency has persisted. 
...
Finally, we fix a compact taxonomy for the candidate operators. 
...
We start by defining the \(\fL\) and \(\fT\) quasiparticle distribution amplitudes. Throughout, we reserve \(f\)-type symbols for quasiparticle
(with \(f\) an occupation probability but
...
[Kopnin's Eqs.~(10.69)--(10.71)], a relation that anchors the sign
convention independently of the branch labelling. Here
\(N_0=m p_{\mathrm F}/2\pi^2\hbar^3\) is the single-spin normal-state
density of states at the Fermi level, used throughout; it is Kopnin's
...
explicitly as \(\mathcal{R}[\fL]\). To be explicit here, we are now in the sector with a real gap, no superflow, no electric field, continuum energies
\(E>\Delta\), and before any angular averaging. The two mode scalar kinetic equations then read:
\begin{align}
  \DOS\,\partial_t\fL
...
We now record the branch-variable map underlying a single Boltzmann-like
equation. The map is exact within the local positive-energy quasiparticle
approximation. Its use below---with the full, unlinearized collision integral and the
convective spectral-flow derivative promoted above---is an adiabatic
completion of the two-mode description, not a literal algebraic
recombination of Kopnin's printed, linearized equations. We assume that the
...
\(k\to k_{\mathrm F}\), to quasiclassical accuracy; the error estimate
appears below, and \cref{app:change_of_variables} keeps general \(k\).)
...
This inverse relation establishes equivalence for the projected equations
just defined; in particular, it should not be read as promoting the
printed linearized two-mode equations to a nonlinear theorem.
...
inherit it. For the even angular harmonics, Kopnin's transverse amplitude
is not \(\phi_T\) itself but the \(\lambda_{\bk}\)-weighted combination
...
\cref{eq:intro_fT_kopnin}. The odd harmonics instead obey the sector swap
of \cref{eq:intro_parity_dictionary}. (Had
...
resulting parity dictionary is worth displaying. Writing
...
\(\phi_T\). For the even, charge-imbalance harmonics of Kopnin's transverse
