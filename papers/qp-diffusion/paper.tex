...
Off-diagonal branch
coherences are generated only at second Wigner order, \(O(\hbar^2)\),
sourced by the energy mode alone---with coefficients proportional to
\(\ddot\Delta\) and \(\dot\Delta^2\) for distributions whose time
dependence is inherited from the gap (SM~\cref{SM-app:nonadiabatic})---and,
being off-diagonal, they do not enter the conserved scalar densities;
their relaxation feeds the collision-integral sector rather than \(\fL\)
directly. Away from \(\xi=0\), the moving-\(\xi\) scalar law is fixed by exact
