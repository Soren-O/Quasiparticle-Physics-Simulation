...
a spurious drift term sourced by the spectral weight inside the divergence;
...
quasiclassical propagator \(\checkg(E,\hat{\bp},\br,t)\), resolved in
the Fermi-surface direction \(\hat{\bp}\) of the momentum,
\begin{equation}
...
\(\Delta\), and \(\otimes\) denotes the Moyal product over the internal
time and Wigner-energy arguments,
\(A\otimes B=A\,\exp\!\bigl[\tfrac{\ii\hbar}{2}
(\overleftarrow{\partial}_{E}\overrightarrow{\partial}_{t}
-\overleftarrow{\partial}_{t}\overrightarrow{\partial}_{E})\bigr]B\)
with the retarded prescription \(E\to E+\ii0^{+}\)
[SM~\cref{SM-eq:moyal_def}], which reduces to the ordinary product at
leading gradient order. Because the \(\xi_{\bk}\) integration at fixed
\(\hat{\bp}\) runs over both signs of \(\xi_{\bk}\), the label
\(\hat{\bp}\) is branch-independent and the streaming velocity is the
normal-state \(\vF\hat{\bp}\): the electron-like (upper Nambu) component
of \(\checkg\) streams along \(+\vF\hat{\bp}\) and the hole-like (lower)
component along \(-\vF\hat{\bp}\). The trajectory label
\(\hat{\bp}=\sigma\hat{\bk}\) of the branch-projected occupation
introduced in \cref{eq:intro_branch_traj} coincides with this
Fermi-surface direction on the electron-like branch and is its antipode
on the hole-like branch; full-sphere averages over the two labels
coincide [SM~\cref{SM-eq:antipodal_average_identity}].
...
streaming term with the sign that makes the electron-like component
stream along \(+\vF\hat{\bp}\); multiplied through by \(\ii\), the
equation takes the Belzig--Kopnin form
\(\ii\hbar\vF\,\hat{\bp}\cdot\widetilde{\bnabla}_{\br}\checkg
+[E\tau_3-\hatD-\check{\Sigma}_{\mathrm{coll}},\checkg]_{\otimes}=0\)
used in the Supplemental Material
[SM~\cref{SM-eq:keldysh_eilenberger}].
...
local gap. Two distinct conditions enter, and they are not
interchangeable. The branch projection behind the Boltzmann form
(no interbranch transitions along a trajectory) requires
\(\hbar|\xi_{\bk}\dot\Delta|/E^3\ll1\), a bounded parameter, at most
\(2\hbar|\dot\Delta|/(3\sqrt3\,\Delta^2)\), that vanishes at the gap
edge. Using the fixed-energy instantaneous-BCS spectral functions
themselves requires the spectral angle to change slowly on the local
scale \(W=\sqrt{E^2-\Delta^2}\), \(\hbar E|\dot\Delta|/W^3\ll1\)
(SM~\cref{SM-app:nonadiabatic}), a pointwise condition that exceeds the
branch parameter on shell by \(\DOS^4\) and fails inside an edge layer
\((E-\Delta)/\Delta\lesssim\tfrac12(\hbar|\dot\Delta|/\Delta^2)^{2/3}\),
where the moving-shell description of \cref{sec:tdep_spectral_flow},
lifetime broadening, or self-consistency must take over. The distribution still feeds back on the spectrum, but only
...
angular derivative of \(\fL\). These equations are applicable in the sector with a real gap, no superflow, no electric field, continuum energies
...
carries the spatial current. In \cref{eq:beta_twomode_T} the transverse amplitude enters only through
the collision functional \(J_T\), so that equation fixes \(\fT\) at each
instant rather than propagating it. That is a consequence of the
quasistatic condition recorded above, not an independent fact about the
display: whenever the transverse relaxation carried by \(J_T\) is fast
compared with the frequencies carried by \(\fT\), i.e.\ \(\omega\tautr\ll1\),
the transverse time-derivative block is negligible beside it. Under that
inequality the anisotropic (dipole) part of \(\fT\) is ``slaved'' to
\(\bnabla_{\br}\fL\), fixed by the balance of streaming against collisions
rather than evolved; where the inequality fails---at the gap edge, in
particular---the block must be retained and \(\fT\) is evolved. This is the
same argument, and the same small parameter, that yields Fick's law from
the normal-metal Boltzmann equation. 
...
The pair \cref{eq:intro_branch} can be made identical in structure by using the trajectory \(\hat{\bp}=\sigma\,\hat{\bk}\) instead;\footnote{At an interface or under directional injection one must remember that the trajectory label \(\hat{\bp}\) stands for the physical momentum direction \(\hat{\bk}=\sigma\hat{\bp}\), antipodal on the hole-like branch; on that branch it therefore differs from the Fermi-surface label \(\hat{\bp}\) of the matrix equations \cref{eq:intro_eilenberger,eq:intro_usadel_matrix}, which is branch-independent. Full-sphere averages over the two labels coincide [SM~\cref{SM-eq:antipodal_average_identity}].} absorbing the sign, \(\widetilde f_\sigma(E,\hat{\bp},\br,t)\equiv f_\sigma(E,\sigma\hat{\bp},\br,t)\),
\begin{equation}
  \Bigl[\partial_t+\tfrac{\Delta}{E}\dot{\Delta}\,\partial_E
...
\(\avg{\,\cdot\,}_{\hat{\bp}}\) is the average over Fermi-surface directions.
...
gap (valid when \(\Delta\) varies slowly on the coherence length and, at
fixed energy, slowly on the local spectral scale,
\(\hbar E|\dot\Delta|/W^{3}\ll1\) with \(W=\sqrt{E^2-\Delta^2}\)
[SM~\cref{SM-app:nonadiabatic}]; the weaker branch condition
\(\hbar|\xi_{\bk}\dot\Delta|/E^{3}\ll1\) controls only the neglect of
interbranch coherence), with
...
direction-resolved quasiclassical transport equation for the matrix
Green's function \(\checkg(E,\hat{\bp},\br,t)\), with \(\hat{\bp}\) the
Fermi-surface direction of the momentum,
...
with the time-gradient corrections otherwise neglected; the expansion
parameter is \(\hbar E|\dot\Delta|/W^3\), SM~\cref{SM-app:nonadiabatic}), so reducing to
...
drives the quasiparticle density of a separate passive probe out of the
well, at \(\DN\DOS^{-2}\bnabla\DOS\), whereas the microscopic operator
