...
\begin{abstract}
Which energy-resolved diffusion operator governs a nonequilibrium
quasiparticle occupation when a superconducting gap varies in space?
...
reduction---are related by an exact change of variables. In the scalar
construction, impurity coherence factors enforce \(\DOS\DE=\DN\); both
routes then yield the conserved spectral density \(\DOS f\) and the
undressed current \(-\DN\bnabla f\). The \(\DOS^2\) dressing instead
belongs to the transverse charge-imbalance channel. The alternative placement
\(\bnabla\cdot[(\DN/\DOS)\bnabla f]\), used as the energy-resolved
starting point in a published gap-engineered-trap analysis, is not
selected by either reduction and produces a spurious
density-of-states-gradient drift for smoothly varying local-BCS gaps.
We derive the common-phase, above-gap scalar Kupriyanov--Lukichev
boundary law and the time-dependent spectral-flow completion, organize candidate
operators in a two-exponent taxonomy, and benchmark prescribed and
self-consistent profiles. The benchmarks demonstrate passive inward
probe drift for the legacy placements and identically zero
density-of-states-gradient drift for the microscopic operator; they do
not imply net nonlinear focusing or negate physical trapping by
gap-edge blocking and inelastic relaxation. We delimit the gap-edge,
proximity, supercurrent, nonadiabatic, and charge-imbalance regimes in
which the coupled two-mode or full matrix equations must be retained.
\end{abstract}
...
Modeling nonequilibrium quasiparticles with spatial resolution in a
superconducting device turns on a question that sounds innocuous:
...
Bardeen--Rickayzen--Tewordt (BRT) coherence-factor cancellation produces
...
Once \(\Delta\) varies in space, the placements separate already at
leading order in \(\bnabla\Delta\): at fixed energy the Fick placement
acquires a density-of-states-gradient drift, divergent toward the local
gap edge, that drives a passive distribution down the gap gradient.
The microscopically selected operator carries no such fixed-energy
drift. Spatial gap variation occurs both in engineered traps and under
self-consistent gap feedback; the net transport effect nevertheless
competes with ordinary diffusion and depends on the profile and
amplitude.
...
into a two-exponent operator family \(L_{p,q}\) (\cref{sec:taxonomy}),
the Kupriyanov--Lukichev interface condition is projected onto the same
...
through \(\delta N=-N_0\!\int_{-\infty}^{\infty}\!\langle\DOS\,\fT\rangle_{\hat\bp}\,\dd E\)
...
definition fixes the same single-spin normalization (his own symbol
\(N_0\) in those equations denotes instead the equilibrium electron
density) \cite{kopnin2001}---used for this quantity throughout, fixing the sign
...
approximation. Its use below with the nonlinear collision law and convective
spectral-flow drive is the adiabatic completion described above, not a literal
algebraic recombination of the printed linearized Kopnin rows. We assume that the
...
just defined; it does not promote the printed linearized two-mode rows to a
nonlinear theorem.
...
mean free path, defined independently of the energy-resolved quasiparticle
\(\elltr(E)=\vg(E)\tautr(E)\) of the scalar closure (the two in fact
...
in the introduction [\cref{eq:intro_three_f}]:
\begin{equation}
  \partial_t f
  +
  \frac{\Delta}{E}\dot{\Delta}\,\partial_E f
...
the introduction. Here the positive-energy branch is selected in the
...
channel (symbolically verified identities I7/I8,
SM~\cref{SM-sec:resolution}): the longitudinal spatial current is
...
\cref{sec:program} does at a material boundary). Finite Dynes broadening is a
...
(SM~\cref{SM-app:nonadiabatic}). Once that correction is included, the
off-diagonal coherence-channel residual is sourced by the energy mode alone
for a generic independently driven \(\fL(E,t)\), with no \(\fT\) source, but
cancels exactly for every shell-slaved distribution \(\fL=G(W)\), including
\(\fL=1\). This collisionless residual is not itself a coherence amplitude;
...
\cref{eq:generic_energy_current}], the occupation obeys
...
This section states the regime boundaries of the shared reduction,
derives the interface reduction of the matrix boundary condition,
exhibits the gap-feedback response that separates the candidate
...
[\cref{eq:intro_parity_dictionary}], so the branch route's
...
\subsection{Conserved currents and boundary conditions}
...
two electrodes (not the Kopnin mode indices of Sec.~I). The interface
...
For the legacy placement C, \((p,q)=(0,-1)\), the first moment of its
conserved variable \(f\) has a fixed-energy drift. A compact local
diagnostic compares that drift with diffusion over a length
\(L_{\Delta}\), where
\(|\delta\Delta|\simeq L_{\Delta}|\partial_x\Delta|\):
\begin{align}
  |v_C|
  &=\frac{\DN\Delta|\partial_x\Delta|}
          {E\sqrt{E^2-\Delta^2}},\\
  \mathrm{Pe}_E
  &\equiv\frac{|v_C|L_{\Delta}}{\DE}
...
\(\Delta(x,t)\) alongside the identities of \cref{sec:tdep_spectral_flow};
script \texttt{verify\_tdep\_inhomogeneous.py} in the verification
repository). Self-consistency changes
...
\begin{figure}[t]
  ...
  \cref{eq:scalar_BC_energy} (interface conductance in diffusion units,
...
\cref{sec:self_consistent_feedback} makes the distinction concrete: for
