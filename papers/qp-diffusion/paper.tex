...
In this route the angular average is performed first, at the matrix
level, and the scalar quasiparticle content is extracted only afterward
by Nambu projection---the opposite order of operations to
\cref{sec:scalar_diffusion}.
...
\end{equation}
where \(\mathcal{P}_{\qp}\) denotes the positive-branch projection (with
the branch-to-trajectory relabelling) that extracts the scalar occupation
\(f(E,\hat\bp)\) from \(\checkg\), and \(\avg{\cdot}_{\hat\bp}\) is the
Fermi-surface angular average.
