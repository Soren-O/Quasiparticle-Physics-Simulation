...
\begin{abstract}
To understand quasiparticle dynamics in non-equilibrium superconductivity, we need to know how quasiparticles transport through the metal. In the dirty limit we expect some type of diffusion equation for the quasiparticles.  The ordinary diffusion equation reads \( \partial_t n = \bnabla\cdot(D\,\bnabla n) \). In the limit where quasiparticle density can be treated as a classical scalar, which energy-resolved diffusion operator governs a nonequilibrium
quasiparticle occupation when the superconducting gap may vary in space and time? Various operators have been used inconsistently in the literature. 
...
and \(\DN\) the normal-state diffusion coefficient. Two examples of alternative diffusion operator that have been used in published gap-engineered-trap
analysis are \(\bnabla\cdot[(\DN/\DOS)\bnabla f]\) and \(\partial_t\chi_{\qp}=(\DN/\DOS)\nabla_{\br}^{2}\chi_{\qp}\), with
\(\chi_{\qp} \propto\DOS f\) the density carried by quasiparticles near a given
energy.  Neither of these are selected by either reduction and
both produce a spurious density-of-states-gradient drift for smoothly
varying local-BCS gaps.
In addition to deriving the diffusion equation, we also derive the common-phase, above-gap scalar Kupriyanov--Lukichev
boundary law, and we derive the time-dependent spectral flow required when the gap varies in time.
\end{abstract}
...
Modeling energy-resolved nonequilibrium quasiparticles with spatial resolution in a
...
gap-engineered traps are transport-engineering responses to them
\cite{martinis2009,riwar2016,hosseinkhani2018,riwar2019,dong2022}. In
...
The prototypical examples of spatial treatments which are exceptions to this are superconducting tunnel-junction photon
spectrometers \cite{kozorezov2002,segall2004} and normal-metal or
...
divergence, \(\bnabla_{\br}\cdot[(\DN/\DOS)\bnabla_{\br}f]\), and the
resulting ansatz is a type of ``Fick's Law'' for the occupation. A
transmon quasiparticle-diffusion measurement \cite{dong2022} writes
the same energy-resolved coefficient (quoted for aluminum) without an
accompanying transport operator. And in Ref.~\onlinecite{martinis2009} the same coefficient is used for a Fick's law for the quasiparticle \emph{density} instead of the occupation,
\(\partial_t\chi_{\qp}=(\DN/\DOS)\nabla_{\br}^{2}\chi_{\qp}\), with
\(\chi_{\qp} \propto\DOS f\) the density carried by quasiparticles near a given
energy.

For a spatially uniform gap these differing choices may reduce to the same result. This is partially why the literature has been able to use different operators inconsistently for so long. 
...
leading order in \(\bnabla\Delta\). The net
transport effect of differing diffusion operators may compete with the ordinary diffusion depending on profile and amplitude.

This paper settles the correct choice of diffusion operator (i.e., the placement of the coefficient from the microscopic theory) by both
...
with the sector statement under which the operator selection is
claimed; \cref{sec:scalar_diffusion,sec:usadel_route} carry out the
scalar-Boltzmann and matrix-Usadel derivations; and
\cref{sec:conserved_currents} projects the Kupriyanov--Lukichev
interface condition onto the scalar current.

Before we proceed we end on a closing note about a useful taxonomy for organizing the various different operators. 
Let \(L_{p,q}[f]=\DOS^{-p}\bnabla_{\br}\cdot(\DN\DOS^{q}\bnabla_{\br}f)\), be the two-exponent family
that organizes the candidate placements. The operator we will end up deriving is then the 
\(L_{1,0}[f]\) one, and we will show other choices of \((p,q)\) have a spurious
drift carried by the spectral weight inside the
divergence (\(q\neq0\)).
...
combination \(\fT=\lambda_{\bk}\phi_T\), with \(\lambda_{\bk}\) the
spectral weight of \cref{eq:beta_sigma_lambda}---not the bare branch
difference \(\phi_T\). The identity holds for the \emph{even} angular
harmonics only (the odd harmonics swap sectors,
\cref{eq:intro_parity_dictionary}), and the two objects are kept
carefully distinct from \cref{eq:intro_branch_traj} onward.
...
momentum-derivative terms of next order in \((k_F\xi_0)^{-1}\); because one of the questions below is how the
