...
\(\DOS\,\DE=\DN\) (\(\DOS\) the BCS density-of-states factor;
\(\tau_{\mathrm N}\), \(\DN\) the normal-state transport time and
diffusion constant; \(\DE\) the energy-resolved diffusivity). Both
routes then give the same operator for the quasiparticle occupation
\(f\), with
conserved spectral density \(\DOS f\), spatial spectral current
\(-\DN\bnabla f\) undressed above the local gap edge and blocked
...
spectral angle, and the scalar mode it dresses.\footnote{Finite
