...
\((2R_bA)^{-1}[\check{g}_1,\check{g}_2]^{K}\), with trace normalization
...
superscripts label the two electrodes (not the Kopnin mode indices of
Sec.~I). The three objects of
...
occupation \(f\) jumps, and matching the interface current to the
conductivity-weighted bulk flux \(-\sigma_N\,\mathcal{D}_L\bnabla\fL\) on
each side (\(\sigma_N=2e^2N_0\DN\) the normal-state conductivity;
equivalently to \(-\DN\,\mathcal{D}_L\bnabla\fL\) with the conductance
expressed in diffusion units, \(G_N/2e^2N_0\)) yields a Robin
condition. (The statements hold equally for \(f\) via \(\fL=1-2f\); the
...
SM~\cref{SM-sec:boundary_conditions}.
...
\begin{figure}[t]
  ...
  \(g_N=G_N/2e^2N_0=0.1~\mu\mathrm{m}/\mathrm{ns}\)), shown at a fixed energy
