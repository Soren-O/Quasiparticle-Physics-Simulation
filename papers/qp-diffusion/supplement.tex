...
\begin{equation}
  ...
  \label{eq:norm_RAK}
\end{equation}
where ``\(\circ\)'' (written \(\otimes\) in the main text) is the
...
\begin{itemize}
  ...
  Equivalently, dividing by \(\sigma_i\), the diffusive flux
  \(-\DN\,\hat n\!\cdot\!\bnabla\fL\big|_i\) matches \(j_L\) with the
  conductance expressed in diffusion units, \(g_N=G_N/(2e^2N_0)\) (the
  dimensionless form used in the benchmark problems). \(\fL\) is
