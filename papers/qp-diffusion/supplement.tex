...
\(\hat n\) the interface normal directed from electrode 1 into electrode 2,
...
\begin{itemize}
  ...
  \item \emph{jump variable}: the bare longitudinal amplitude \(\fL\); the
  quantity continuous across the interface is the matrix current --- hence
  \(j_L\) --- \emph{not} \(\fL\) or \(\DOS\fL\). Taking the same plain trace
  of the left-hand side of \cref{eq:KL_BC} as in the bulk matches
  \cref{eq:scalar_BC_boxed} to the bulk flux on each side,
  \(-\sigma_i\,\mathcal D_L\,\hat n\!\cdot\!\bnabla\fL\big|_i=j_L\)
  (\(\mathcal D_L=1\) above both gaps), a Robin condition: the normal
  derivative of \(\fL\) is tied to the weighted amplitude jump.
  Equivalently, dividing by \(2e^2N_0^{(i)}\), the diffusive flux
  \(-\DN\,\hat n\!\cdot\!\bnabla\fL\big|_i\) matches \(j_L\) with the
  conductance expressed in diffusion units, \(g_N=G_N/(2e^2N_0)\) (a
  velocity; the form used in the benchmark problems). \(\fL\) is
