...
\end{equation}
The instantaneous propagator therefore fails the spectral equation only at
\(O(\hbar^2\ddot\Delta)\), with a purely off-diagonal (\(\tau_1\)) residual;
the density of states and the spectral coefficients
\(\mathcal D_L,\mathcal D_T\) are exact through \(O(\hbar)\), receiving
corrections---which lie in the \(\tau_3/\tau_2\) sector and carry both
\(\ddot\Delta\) and \(\dot\Delta^2\) pieces---first at \(O(\hbar^2)\).
...
\end{equation}
(a spurious \(O(\hbar)\) drive, proportional to
\(E\dot\Delta\,\partial_E\fL+\Delta\,\partial_t\fL\), appears here only for
the opposite advanced-propagator sign). The leading branch-coherence
generation appears at \(O(\hbar^2)\), sourced by the energy mode alone (no
\(\fT\), including its derivatives)---the same order as the retarded
spectral residual \cref{eq:nonad_residual}. For a distribution whose time
dependence is inherited from the gap (each time derivative of \(\fL\)
carrying a factor \(\dot\Delta\)) its coefficients are proportional to
\(\ddot\Delta\) and \(\dot\Delta^2\); for an independently driven
\(\fL(E,t)\) the source also carries
\(\dot\Delta\,\partial_E\partial_t\fL\) and \(\Delta\,\partial_t^2\fL\)
pieces. Being off-diagonal it
...
\(O(\hbar^2)\) (for gap-slaved distributions, through \(\ddot\Delta\) and
\(\dot\Delta^2\), vanishing as \(\dot\Delta,\ddot\Delta\to0\)), recovering
main-text~\cref{M-eq:usadel_A1_tdep_fL}.
