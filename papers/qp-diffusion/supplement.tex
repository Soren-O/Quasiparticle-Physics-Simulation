...
(\cref{sec:slaving}) produce a real coefficient. Other references keep the
streaming term real and let the impurity self-energy carry the factor of
\(\ii\) instead; the two forms differ by a global rescaling of the
equation, not a different physical theory,
and the matrix Keldysh--Usadel form
\cref{eq:matrix_keldysh_usadel} is invariant. (The main-text starting
display, \cref{M-eq:intro_eilenberger}, corresponds to the opposite sign
convention for the Wigner \((E,t)\) kernel---equivalently the conjugated
equation, with real streaming and the \(\ii\) carried by the full
commutator; the reduction below is insensitive to that choice as well.)
...
leading gradient order reduces to the ordinary commutator plus a
Poisson-bracket-type gradient correction, written schematically
\([A,B]_{\circ}\to[A,B]+(\ii\hbar/2)\{A,B\}_{\mathrm{P.B.}}\)
in mixed (Wigner) variables and made precise for matrix-valued operators
(where the four-product ordering matters) in \cref{eq:moyal_comm}. The Poisson bracket on the
...
\begin{equation}
  ...
  \label{eq:S_result}
\end{equation}
The \(O(\Gamma)\) expansion is valid for \(W^3\gg\Gamma E\Delta\), i.e.\
...
to the ideal bulk BCS background, where \(\sinh^2\theta\) is real at
...
energies in such broadened spectra---\(S(E)=2Q\,\Imag\sinh^2\theta_0(E)+O(Q^3)\) is generically
...
verification extends the statement through $O(\hbar^2)$. Second, absent
externally injected imbalance, any indirect path requires $\fT$ to be
generated by the gap drive and then fed back. The transverse equation of \cref{sec:transverse_projection} makes the
