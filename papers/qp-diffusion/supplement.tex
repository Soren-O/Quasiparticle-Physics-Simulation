...
\begin{equation}
  ...
  \label{eq:norm_RAK}
\end{equation}
where ``\(\circ\)'' (the main text's \(\otimes\), up to the kernel-sign
convention noted below \cref{eq:keldysh_eilenberger}) is the
...
\]
which fixes the factors of two and the single-spin convention for \(N_0\).
...
throughout, all currents understood as components along it). For the \emph{frozen} BCS spectral functions
...
traces to zero in the energy mode (verified symbolically for generic
