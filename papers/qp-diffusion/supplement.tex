...
\usepackage{xcolor}
\usepackage{xr-hyper}
% See the matching note in paper.tex: snapshot labels take precedence when
% generated (make arxiv-labels); live xr import otherwise.
\IfFileExists{xr-labels-from-paper.tex}
  {\makeatletter\input{xr-labels-from-paper.tex}\makeatother}
  {\externaldocument[M-]{paper}[paper.pdf]}
...
\title{Supplemental Material for ``A Diffusion Equation for Nonequilibrium Superconducting Quasiparticles''}
...
\(f(E,\br,t)\) valid in the dirty limit \(\tau\Delta/\hbar\ll 1\). Target form:
\begin{equation}
...
\[
  ...
  \tau_3\,\mathcal{G}^{R/A/K}\!\bigl(\bp,E,\br,t\bigr),
\]
are \(2\times 2\) matrices in Nambu space depending on energy \(E\), the
Fermi-surface direction \(\hat\bp\) of the momentum (a unit vector along
the normal-state Fermi velocity; the \(\xi_\bp\) integration at fixed
\(\hat\bp\) spans both branches, so \(\hat\bp\) is branch-independent and
is not the trajectory label \(\hat\bp=\sigma\hat\bk\) of the main text's
branch-projected occupation, main-text~\cref{M-eq:intro_branch_traj},
which coincides with it only on the electron-like branch), spatial
...
\begin{equation}
  ...
  \label{eq:norm_RAK}
\end{equation}
where ``\(\circ\)'' (the main text's \(\otimes\)) is the
Wigner-representation Moyal product; in the time--energy pair it reads
\(A\circ B=A\,\ee^{(\ii\hbar/2)(\overleftarrow{\partial}_{\!E}\overrightarrow{\partial}_{\!t}-\overleftarrow{\partial}_{\!t}\overrightarrow{\partial}_{\!E})}B\)
(expanded in \cref{eq:moyal_def}), with spatial gradients kept explicit in
the streaming term. To leading gradient
...
Schmid--Sch\"on--Larkin--Ovchinnikov (SSLO) form
...
\paragraph{Spectral functions.} For a real gap on the positive-energy
quasiparticle branch (time dependence, where present, enters adiabatically
through \(\Delta(\br,t)\)),
\begin{align}
  \DOS(E,\br) &= \Real\,\frac{E}{\sqrt{(E+\ii 0^+)^2-\Delta^2(\br)}}
              = \frac{E}{\sqrt{E^2-\Delta^2(\br)}}\,\theta(E-\Delta), \\
  N_2(E,\br) &= \Real\,\frac{\Delta(\br)}{\sqrt{(E+\ii 0^+)^2-\Delta^2(\br)}}
...
\(\ii 0^+\) limit, \(\Real g^{R}=\DOS\), \(\Real f^{R}=N_2\), and
\(g^{A}=(g^{R})^*\), \(f^{A}=(f^{R})^*\).
...
\(\tau\) (the main text's \(\tau_{\mathrm N}\)) is the elastic impurity
transport time entering the \(s\)-wave Born
self-energy below.
...
(\cref{sec:slaving}) produce a real coefficient. It is the main-text
starting display [main-text~\cref{M-eq:intro_eilenberger}] multiplied by
\(\ii\): both documents describe one and the same
\(\checkg(E,\hat\bp,\br,t)\), with \(\hat\bp\) the Fermi-surface direction
defined above, and therefore share every angular harmonic, in particular
the slaved first harmonic of \cref{eq:slaving}. (An antipodal relabelling
\(\hat\bp\to-\hat\bp\) would map either display onto the one with the
opposite relative sign between streaming and commutator, but it is odd on
the first harmonic, \(\check{\mathbf g}_1\to-\check{\mathbf g}_1\), and
would reverse the matrix current; it is not used.) In the normal-state
limit the equation propagates the electron-like Nambu component along
\(+\vF\hat\bp\) and the hole-like component along \(-\vF\hat\bp\), which
fixes the relative sign of streaming and commutator for the declared
Wigner kernel \cref{eq:moyal_def}.
...
The square-bracket commutator is taken with the Moyal product. For commuting
scalar symbols its leading nonzero term is
\[
  [A,B]_{\circ}
  =
  \ii\hbar\,\{A,B\}_{\mathrm{P.B.}}+O(\hbar^3),
\]
with no factor of \(1/2\). For matrix-valued operators the ordinary
commutator remains and the four first-order products cannot be combined
into a single scalar Poisson bracket; their ordering is given explicitly in
\cref{eq:moyal_comm}. The gradient term on the
...
\end{equation}
where \(E\) ranges over the kinetic window in which the reduced equation
is used (up to a few \(\Delta\), \(k_{\mathrm B}T\)), and \(L\) is the
spatial scale of variation of \(\Delta(\br)\) and of
...
also the condition imposed in the main text to label the resulting
operator as ``Usadel.''
...
parameter \(\ell/L\). The normalization
...
equation), it dominates every other term containing
\(\check{\mathbf{g}}_1\) and is balanced against the streaming source,
which slaves \(\check{\mathbf{g}}_1\).
...
\begin{description}
  ...
  +\ii\,[\,E\tau_3-\hatD-\check\Sigma_{\mathrm{inel}},\,\checkg_0\,]_{\circ}=0\)
  with \(\checkg_0\circ\checkg_0=\openone\).
  \item[\cref{sec:fLfT} Keldysh projection and spatial flux dressing.] Project the
  Keldysh component using
  \cref{eq:dist_matrix_ansatz}. The \(\Tr\) and \(\Tr[\tau_3\,\cdot]\)
  projections dress the two spatial fluxes with the spectral coefficients
  \(\mathcal{D}_{L/T}=\tfrac{1}{4}\Tr[\openone-\hatg^{R}(\tau_3)\hatg^{A}(\tau_3)]\),
  evaluating to \(\mathcal{D}_L=1\) and
  \(\mathcal{D}_T=\DOS^2\) on the bulk BCS background above the gap; the
  time side, completing the two sector-decoupled scalar kinetic equations,
  is assembled in Step 3.
...
The antipodal relabelling leaves the bare angular average invariant,
\cref{eq:antipodal_average_identity}. The representational
difference lies instead in how the odd, current-carrying first harmonic is
retained, slaved, and eliminated before the scalar end point is formed.
The remaining subsections collect the interface boundary conditions and
the beyond-real-gap (supercurrent, proximity), nonadiabatic, and
lifetime-broadened extensions that bound the sector of the main text.
...
homogeneous Fischer--Catelani kinetic equations~\cite{fischer2023}). The elastic impurity
...
written in the Belzig et~al.\ review~\cite{belzig1999} and in
...
commutator in \cref{sec:fLfT,sec:scalar_reduction}; the explicit
calculation, deferred to \cref{sec:scalar_reduction}, shows that the same
expansion also generates the spectral-flow
\((\Delta/E)\dot\Delta\,\partial_E\) term, with the factors and signs
derived there rather than asserted here.
...
retarded spectral equation \([\hat L_0,\hatg^R]_\star=0\), with \(\star\)
the \((E,t)\) Moyal product of \cref{eq:moyal_def}, supply its
...
advanced quasiclassical propagators are (main
text,~\cref{M-eq:bcs_advanced_matrix})
\begin{equation}
  \hatg^{R}
  =
...
the off-diagonal cross-term (\(\tau_1\) in this conjugated form, \(\tau_2\)
in the flux coefficient itself) survives at the matrix level in the transverse
...
\begin{equation}
  ...
  \label{eq:DL_DT_def}
\end{equation}
which are (one quarter of) the Nambu traces of the matrix
coefficients of \(\widetilde\bnabla\fL\) and (after right-multiplication by
\(\tau_3\), i.e.\ under the \(\tau_3\)-trace)
\(\widetilde\bnabla\fT\); we now evaluate them on the BCS bulk. Plugging in \cref{eq:gRgA}, \cref{eq:gRtau3gA}
...
These coefficients define diffusion-normalized spectral mode fluxes for the
longitudinal and transverse sectors,
\begin{equation}
  \mathbf j_L
  =
...
respectively. (Note the projection pairing inverts between densities and
fluxes: the plain trace picks the \(\fT\) density in
\cref{eq:trace_identities} but the \(\fL\) flux here, because the flux
coefficient carries one more factor of the propagator; the Moyal
\(\partial_E(E\tau_3)\) term of the time projection restores the match in
the assembled kinetic equation, \cref{sec:scalar_reduction}.) The longitudinal current is \emph{undressed}, with the bare
...
\(q=0\) for the longitudinal mode in the \(L_{p,q}\)
classification (\cref{eq:Lpq_family}).
...
\(\mathcal D_L=\cos^2(\Imag\theta)\) and
\(\mathcal D_T=\cosh^2(\Real\theta)\), with the exact identity
...
factors in the indicated left--right order. This \(\star\) is the Wigner
\((E,t)\) restriction, expanded to the displayed order, of the Moyal
product written \(\circ\) in the Conventions (and \(\otimes\) in the main
text). The corresponding commutator
is
\begin{equation}
  [A,B]_{\star}
  =
...
(\(\Delta^{(3)},\Delta^{(5)},\ldots\)), genuinely higher order in the
...
\(2N_2\) (assembling, together with the \(\tau_3\) piece and BCS
DOS-continuity, exactly the flux
...
The fixed-\(E\) plain-trace projection above produces the spectral flow in
conservative form, as the energy-space flux
\(\partial_E(\dot\Delta\,N_2\,\fL)\) of \cref{eq:trace_gap}; no bare
advective drift \(\dot\Delta\,\partial_E\) acting on \(\fL\) alone appears
in that operation. There is a precise kinematic reading of the same flux:
it arises once the static Usadel continuity law is lifted from a fixed
spectrum to an instantaneous quasiparticle spectrum (a re-expression of
the fixed-\(E\) result, not an independent completion).
...
density \(\DOS\fL\): differentiating the Jacobian
\(\DOS=\partial\xi/\partial E\) at fixed \(\xi\) gives
\begin{equation}
  \partial_t\DOS+\partial_E(u_E\DOS)=0.
  \label{eq:DOS_continuity_from_jacobian}
\end{equation}
In the matrix language the
...
basis. This supplies an exact scalar coordinate relabelling, not an intrinsic
moving-frame matrix dirty-limit derivation. Away from \(\xi=0\), the
relabelling fixes the moving-\(\xi\) scalar equation, including its spatial
connection terms; the moving-\(\xi\) chart is nonuniform at the gap edge.
Separately, the fixed-\(E\) calculation of \cref{sec:resolution} confirms that
the longitudinal matrix current is the undressed \(\bnabla\fL\), with no
spurious fixed-\(E\) \(\bnabla\Delta\,\partial_E\) drift and no
...
spectral corrections [\(\hbar E|\dot\Delta|/W^3\ll1\),
\(W=\sqrt{E^2-\Delta^2}\), \cref{app:nonadiabatic}], no Landau-Zener-like
branch conversion [\(\hbar|\xi\dot\Delta|/E_\xi^3\ll1\),
\cref{eq:branch_coherence_estimate}], and no unresolved
...
parameter made explicit below [\cref{eq:branch_coherence_estimate}]. To
leading adiabatic order,
\begin{equation}
  (D_t \hat h_\xi)_{++}
  =
...
This fixes the formal status of the construction: it is a hybrid in which the
branch projector supplies the moving spectral time coordinate and the
fixed-\(E\) matrix reduction supplies the spatial current. Exact scalar
relabelling reproduces \cref{eq:branch_projector_A1} away from the gap edge,
but an intrinsic moving-\(\xi\), gauge-invariant-energy, or branch-covariant
matrix dirty-limit derivation has not been performed. The fixed-\(E\)
constant-trace reduction yields the stated scalar content, while the
off-diagonal coherences are suppressed by the adiabatic parameter of
\cref{eq:branch_coherence_estimate}; the instantaneous spectral functions
that the assembled equation imports (the \(\DOS\) weight on the left and
the undressed flux on the right) are controlled by the more restrictive
Moyal parameter \(\hbar E|\dot\Delta|/W^3\) of \cref{app:nonadiabatic},
which is the binding condition on
\cref{eq:branch_projector_A1}. What lies beyond the present assumptions is
no longer fully open: the supercurrent-induced longitudinal--transverse mixing
and the non-BCS proximity dressings are established, while the normalized
spectral correction and collisionless coherence residual give the leading
nonadiabatic content in
\cref{app:supercurrent,app:proximity,app:nonadiabatic}. The nonadiabatic
spatial-current correction and collision-level physics (memory, drive
sidebands, branch-coherence relaxation, or a gap changing appreciably during
a collision) remain open, as does self-consistent gap dynamics.
...
This parameter controls only the interbranch coherence at fixed \(\xi\).
It is globally bounded,
\(\hbar|\xi\dot\Delta|/E_\xi^3\le(2/3\sqrt3)\,\hbar|\dot\Delta|/\Delta^2\)
(the maximum at \(\xi=\Delta/\sqrt2\)), and it vanishes at \(\xi=0\),
where the eigenvectors of \(H_\xi\) do not rotate; it therefore reduces to
the single energy-independent statement \(\hbar|\dot\Delta|\ll\Delta^2\)
and carries no energy resolution. It is \emph{not} the condition for
using instantaneous BCS spectral functions at fixed energy. That
condition, derived in \cref{app:nonadiabatic}, is
\(\hbar E|\dot\Delta|/W^3\ll1\) with \(W=\sqrt{E^2-\Delta^2}\): on shell
(\(|\xi|=W\)) it exceeds the branch parameter by \(\DOS^4=E^4/W^4\), and
it diverges at the gap edge where the branch parameter vanishes. The two
conditions govern different objects. The branch projection follows each
state along its moving energy \(E_\xi(t)\), which is adiabatic even at
the edge; the fixed-\(E\) Wigner representation asks for the propagator
at an energy across which the gap edge itself sweeps. The fixed-\(E\)
spectral functions therefore fail inside an edge layer
\((E-\Delta)/\Delta\lesssim\tfrac12(\hbar|\dot\Delta|/\Delta^2)^{2/3}\),
within which the moving-shell variable \(W\)
(\cref{sec:moving_bin_projection}), lifetime broadening, or
self-consistency must regularize the description.
...
\end{equation}
so the diffusion-normalized transverse matrix-current projection is dressed,
...
transverse dressing of \cref{eq:traceT_diffusion} belongs to the
trace-projected transverse mode flux \(\mathbf j_T\), not to the
kinetic-equation flux in this normalization. It acquires the conductivity
factor and energy integration needed for an observable charge current only
in \cref{sec:boundary_conditions}. Accordingly \(\partial_t(\DOS\fT)\) and
...
\begin{equation}
  \mathcal R
  \equiv
  \frac{\ii}{4}\Tr\!\bigl[\hatD\,(\hatg^R+\hatg^A)\bigr]
  =
  -\Delta\,\Imag\sinh\theta,
  \label{eq:depairing_coefficient}
\end{equation}
where the prefactor \(\ii\) converts the raw, purely imaginary trace into
the real kinetic coefficient used in this gap gauge. Uniform ideal BCS
above the gap (or the zeroth-order local-BCS background) has real
\(\theta\) and hence \(\mathcal R=0\), consistently with the
absence of coherent above-gap relaxation in
\cref{eq:fT_kinetic_conservative}; for positive-energy ideal BCS below the
gap, \(\mathcal R=\Delta^2/\sqrt{\Delta^2-E^2}>0\).
Depairing, proximity, or other complex spectra are covered by the same
general expression. Likewise a supercurrent couples the two channels through
\(S=2Q\,\Imag\sinh^{2}\theta\) (\(Q\) the gauge-invariant superfluid
momentum; derived and verified symbolically in \cref{app:supercurrent}),
which vanishes on the real-\(\Delta\), zero-\(Q\) background of this section.
...
\begin{equation}
  ...
  \widetilde\bnabla\!\cdot\!\bigl[\fT\,\widetilde\bnabla\vartheta_0\bigr]
  +\hbar\,J_T ,
  \qquad
  \vartheta_0=\operatorname{artanh}\frac{\Delta}{E},
  \label{eq:fT_inhom}
\end{equation}
Here \(\vartheta_0\) is the zeroth-order local-BCS angle and
\(\widetilde\bnabla\vartheta_0
=E\,\widetilde\bnabla\Delta/W^{2}\). Alongside the weighted diffusion, a
gap gradient coherently converts charge imbalance, the transverse
counterpart of the statement that the longitudinal channel has \emph{no}
DOS-gradient drift.
...
content, not an independent third scalar equation.

Retaining the curvature in \cref{eq:fT_inhom} also requires the
same-order spectral completion of the strict local-BCS propagator. With
\(\hatg_0=\DOS\tau_3+\ii N_2\tau_2\),
\(\hat m_0=N_2\tau_3+\ii\DOS\tau_2\), and \(W=\sqrt{E^2-\Delta^2}\), define
\begin{equation}
  \eta
  =
  \frac{\hbar\DN}{2W}\,
  \widetilde\bnabla^2\vartheta_0,
  \qquad
  \delta\hatg^R=\delta\hatg^A=\ii\eta\,\hat m_0,
  \qquad
  \delta\hatg^K=-2\ii\eta\,\DOS\fT\,\tau_1 .
  \label{eq:spatial_spectral_completion}
\end{equation}
The correction obeys
\(\{\hatg_0,\delta\hatg^{R/A}\}=0\). Its spectral commutator cancels the
local-current image residual exactly,
\begin{equation}
  \ii[\hat L_0,\delta\hatg^K]
  =
  2\hbar\DN\DOS
  (\widetilde\bnabla^2\vartheta_0)\fT\,\hat m_0 ,
  \label{eq:spatial_image_cancellation}
\end{equation}
while the opposing local-current term has the same magnitude and negative
sign. Both have zero kernel projection, so \cref{eq:fT_inhom} is unchanged.
In the trace representation the completion gives
\begin{equation}
  \mathcal R_{\bnabla}
  =
  -\Delta\DOS\,\eta
  =
  -\frac{\hbar\DN\Delta\DOS}{2W}
   \widetilde\bnabla^2\vartheta_0,
  \qquad
  \tfrac14\Tr\!\left[
    \tau_3\,\ii[\hat L_0,\delta\hatg^K]
  \right]
  =
  -2\mathcal R_{\bnabla}\fT .
  \label{eq:spatial_relaxation_trace}
\end{equation}
This is the trace-form bookkeeping of curvature already present in the
kernel equation, not an additional term to be appended to
\cref{eq:fT_inhom}; doing so would double count it. The diffusion
coefficients \(\mathcal D_L=1\) and \(\mathcal D_T=\DOS^2\) have no
linear-\(\eta\) correction.

The local expansion requires
\(\rho_\Delta\equiv|\eta|\ll1\). It is nonuniform at the ideal gap edge,
since
\[
  \widetilde\bnabla^2\vartheta_0
  =
  \frac{E}{W^2}\widetilde\bnabla^2\Delta
  +
  \frac{2E\Delta}{W^4}
  |\widetilde\bnabla\Delta|^2 .
\]
...
An intrinsic representation-level matrix-Usadel reformulation (with the
moving spectral control volume present before the scalar projection) has not
been carried out. \Cref{sec:resolution} instead verifies the fixed-\(E\)
spatial current directly; exact scalar relabelling then determines the
moving-\(\xi\) scalar form away from the singular gap-edge coordinate.
...
\begin{equation}
  ...
  \label{eq:LD_p1q0}
\end{equation}
the \((p,q)=(1,0)\) member of the \(L_{p,q}\) family
(\cref{eq:Lpq_family}), with \(\mathcal D_L=1\) above the local gap edge.
...
The diagnostic \((p,q)=(2,2)\) operator
...
\(f(E,\hat\bp,\br,t)\) then obeys the semiclassical kinetic equation of
main-text~\cref{M-sec:scalar_diffusion} (the refraction force dropped;
the sector assumptions collected at the end of
main-text~\cref{M-sec:quasiclassical} apply),
\begin{equation}
  \partial_t f
...
\begin{equation}
  ...
  f_0(E,\br,t)
  +
  \hat\bp\cdot\mathbf a(E,\br,t)+\cdots
  \label{eq:branch_harmonic}
\end{equation}
(written \(\mathbf a\), not \(\mathbf f_1\), for the same reason as in the
main text). For an isotropic elastic kernel, the first angular harmonic relaxes with the
transport rate
\begin{equation}
  \frac{1}{\tautr(E)}
  =
...
\(\xi\to-\xi\), \(u\) and \(v\) are interchanged and the same charge-like matrix
...
\(\DOS^{-2}(E)\). Relative to the normal-state transport rate
\(\tau^{-1}\) (\(\tau\) as in the Conventions; the main text's
\(\tau_{\mathrm N}\)),
...
\end{equation}
up to the same angular transport factor that defines \(\tau\). Since
\(\vg(E)=\vF/\DOS(E)\), the mean free path is
\begin{equation}
  \elltr(E)=\vg(E)\tautr(E)=\vF\tau,
...
\begin{equation}
  D_{\mathrm{BRT}}(E)
  =
  \frac{\vF^2\tau}{d}\,\frac{1}{\DOS(E)}
  =
  \frac{\DN}{\DOS(E)} .
  \label{eq:branch_brt_diffusion}
\end{equation}
This is the Bardeen--Rickayzen--Tewordt (BRT) clean-elastic
cancellation~\cite{bardeen1959} in the
...
\cref{eq:Lpq_expanded}; they are not the end point of the
branch reduction.

The two routes therefore coincide, although their first-harmonic closures
are represented differently. In the branch route, one projects onto a quasiparticle with
...
bare angular average itself commutes with the antipodal relabelling,
\cref{eq:antipodal_average_identity}. That relabelling acts on
odd angular harmonics, so the branch route's current-carrying
...
the difference is in retaining and eliminating the first harmonic, not in
the full angular average or the diffusion law.
...
the static dirty-limit ordered operator of
\cref{eq:branch_diffusion_coefficient}, landing on the A1 operator of
\cref{eq:LD_p1q0} for the BRT coefficient \cref{eq:branch_brt_diffusion};
the constant-\(\tau\) diagnostic \cref{eq:branch_constant_tau_diffusion}
enters the same ordered form as its \((1,-1)\) member.
...
same longitudinal amplitude used in the bulk.

\paragraph{Matrix relation.} For two diffusive electrodes \(1,2\) joined by a
low-transparency barrier, let \(R_b\) be the measured normal resistance of
a patch of area \(A\), and define its specific resistance and conductance
density by
\(\mathcal R_b\equiv R_bA\) and \(G_N\equiv\mathcal R_b^{-1}\). Then
\begin{equation}
  \sigma_i\,\check g_i\,\hat n\!\cdot\!\widetilde\bnabla\check g_i
  =\frac{G_N}{2}\,[\check g_1,\check g_2],
  \label{eq:KL_BC}
\end{equation}
with \(\sigma_i=2e^2N_0^{(i)}\DN^{(i)}\) the normal-state conductivity of
electrode \(i\) (\(N_0\) the single-spin Fermi-level density of states) and
\(\hat n\) the common directed normal from electrode 1 into electrode 2
(the outward-normal form has opposite signs on the two sides),
so the matrix current through the barrier is the commutator
\(\check I=(G_N/2)[\check g_1,\check g_2]\), continuous across the interface.
The measured \(G_N\) includes both spin channels.

We now restrict the scalar evaluation to rigid ideal BCS spectra, a common
condensate phase, and \(E>\max(\Delta_1,\Delta_2)\). In this sector take
...
\paragraph{Interface and diffusion normalizations.}
The conductivity-weighted spectral mode currents directed from electrode
1 to electrode 2 are
\begin{equation}
  \boxed{\;
    \mathcal J_L
    =
    G_N\,\big[\DOS^{(1)}\DOS^{(2)}-N_2^{(1)}N_2^{(2)}\big]\,
    \big(\fL^{(1)}-\fL^{(2)}\big)
  \;}
...
\begin{equation}
  ...
  G_N\,\DOS^{(1)}\DOS^{(2)}
  \big(\fT^{(1)}-\fT^{(2)}\big).
  \label{eq:scalar_BC_T_current}
\end{equation}
The first weight is the Maki--Griffin common-phase heat-current factor
\cite{maki1965}. The interface law contains a dimensionless spectral
weight, the conductance density \(G_N\) (units \(\mathrm{S\,m^{-2}}\)),
and the amplitude jump. It must not be identified term by term with the
bulk mobility in \cref{eq:LD_p1q0}.

For either \(a=L,T\), reserve \(j_a^{(i)}\) for the velocity-unit,
diffusion-normalized flux on side \(i\):
\begin{align}
  \mathcal J_a
  &=
  -\sigma_i\,\mathcal D_a^{(i)}
  \hat n\!\cdot\!\bnabla f_a^{(i)}
  =
  2e^2N_0^{(i)}j_a^{(i)},\\
  j_a^{(i)}
  &\equiv
  ...
  \label{eq:KL_side_normalizations}
\end{align}
Here \(f_L\equiv\fL\), \(f_T\equiv\fT\), and
\(\mathcal W_{L,T}\) are the two weights in
\cref{eq:scalar_BC_boxed,eq:scalar_BC_T_current}.
Only \(\mathcal J_a\) is continuous across dissimilar materials. For a
common planar area,
\[
  \frac{j_a^{(1)}}{j_a^{(2)}}
  =
  \frac{N_0^{(2)}}{N_0^{(1)}} ,
\]
so a side-independent \(g_N\) is valid only when the normal-state densities
of states agree. If cross-sectional areas differ in a one-dimensional
reduction, the conserved object is the total \(A_i\mathcal J_a^{(i)}\).
A nonzero current generically produces a finite amplitude jump. Rewriting
the longitudinal Robin law in terms of \(f=(1-\fL)/2\) multiplies its bulk
and interface sides by the same factor and leaves the coefficient and
spectral weight unchanged. The formal \(G_N\to\infty\) Robin limit enforces
equal amplitudes, but is outside the controlled low-transparency K-L
derivation; the resulting continuity condition agrees with the
transparent-interface limit.

\paragraph{Observable currents and normal-state check.}
With \(e>0\) and the orientation above, the observable charge- and
heat-current densities are
\begin{align}
  J_{\mathrm{ch}}
  &=
  \frac{1}{2e}\int_{-\infty}^{\infty}
  \mathcal J_T(E)\,\dd E,\\
  J_Q^{1\to2}
  &=
  -\frac{1}{2e^2}\int_{-\infty}^{\infty}
  E\,\mathcal J_L(E)\,\dd E
  =
  -\frac{1}{e^2}\int_0^\infty E\,\mathcal J_L(E)\,\dd E .
  \label{eq:KL_observable_currents}
\end{align}
The second equality for \(J_Q\) uses the particle--hole parity
\(\mathcal J_L(-E)=-\mathcal J_L(E)\) of the present sector.  The
heat-current minus sign follows from \(\fL=1-2f\); reversing
\(\hat n\) reverses all three directed currents together. Total electrical
and heat currents are \(I=A J_{\mathrm{ch}}\) and
\(\dot Q=A J_Q\). In the normal limit these formulas recover
\[
  \frac{I}{A}=G_N(V_1-V_2),
  \qquad
  \frac{K_N}{A}
  =
  \frac{\pi^2 k_{\mathrm B}^2T}{3e^2}\,G_N ,
\]
which fixes the factors of two in
\cref{eq:KL_BC,eq:scalar_BC_L,eq:scalar_BC_T} and in the observable-current
prefactors above. Neither identity involves \(N_0\); its single-spin
convention is the one stated with \(\sigma_i\) beneath \cref{eq:KL_BC}.
No explicit \(\hbar\) belongs in these stationary formulas: \(E\) is an
energy, \(N_0\) is per energy-volume, and \(\hbar\) is already implicit in
\(N_0\) and the microscopic barrier resistance.
...
density-of-states product familiar from SIS quasiparticle tunneling (the standard coherence-factor distinction between energy and
...
down the amplitude difference.

\paragraph{Scope beyond rigid above-gap BCS.}
For a common phase but generic complex spectra, writing \(g_i^R\) and
\(f_i^R\) for the scalar normal and anomalous components gives the diagonal
weights
\[
  \mathcal W_L
  =
  \Real g_1^R\,\Real g_2^R
  -
  \Real f_1^R\,\Real f_2^R,
  \qquad
  \mathcal W_T
  =
  \Real g_1^R\,\Real g_2^R
  +
  \Imag f_1^R\,\Imag f_2^R .
\]
Thus the DOS product printed in
\cref{eq:scalar_BC_T,eq:scalar_BC_T_current} is specifically the ideal
above-gap limit. For \(\fT=0\), a phase difference
\(\delta\chi=\chi_1-\chi_2\) changes the longitudinal weight to
\(\DOS^{(1)}\DOS^{(2)}
-N_2^{(1)}N_2^{(2)}\cos\delta\chi\).
For generic complex spectra the full finite-phase two-mode law also contains
\(\sin\delta\chi\) longitudinal--transverse mixing terms. That phase-biased
extension is not used in the common-phase interface law or benchmark here.

\paragraph{Gap-edge caveat.} In the rigid common-phase BCS sector, both
weights inherit an integrable
...
\(\tfrac14\Tr[\check J^K]=\bnabla\fL\),
\(\check J^{K}\equiv(\checkg\,\bnabla\checkg)^{K}\), which carries no
\(\fT\)) is
special to a real gap. A supercurrent reopens the coupling (on the
ideal BCS background, with a nonuniform small-\(Q\) edge response). With
a phase gradient the gauge-covariant gradient is
\(\widetilde\bnabla\hatg=\bnabla\hatg-\ii Q[\tau_3,\hatg]\), \(Q\) the
gauge-invariant superfluid momentum (taken along a single direction
throughout, all currents understood as components along it). For the \emph{frozen} BCS spectral functions, above the gap (\(E>\Delta\), where \(\hatg^A=-\hatg^R\)),
...
scalar channels. In this above-gap sector the longitudinal
...
\(\Gamma=\tfrac12\hbar D q^2=2\hbar D Q^2\) (\(q=2Q\) the gauge-invariant
pair-phase gradient)~\cite{belzig1999}, so that with
...
with \(\theta=\theta_0-\ii\Gamma\sinh\theta_0\cosh\theta_0/W\)
(\(\theta_0=\theta_{\mathrm{BCS}}\)) and
\(W=\sqrt{E^2-\Delta^2}\),
\begin{equation}
  \Imag\,\sinh^2\theta=-\frac{2\Gamma E^2\Delta^2}{W^5},
  \qquad
  \boxed{\,S_{\mathrm{out}}(E;Q)
       =-\frac{4Q\Gamma E^2\Delta^2}{W^5}
       =-\frac{8\hbar D\,Q^3 E^2\Delta^2}{W^5}
       +O(Q^5)\,}.
  \label{eq:S_result}
\end{equation}
This fixed-energy outer expansion is valid for
\(W^3\gg\Gamma E\Delta\), i.e.\
outside the depairing-rounded edge region
\(E-\Delta\sim\Delta\,(\Gamma/\Delta)^{2/3}\), where the rounded angle of
\cref{eq:depairing} must be used directly.
The expansion is nonuniform in that layer. Since \(\Gamma\propto Q^2\),
its width is \(O(\Gamma^{2/3})=O(Q^{4/3})\), while the peak of
\(|S|\) is \(O(Q^{-1/3})\). The integrated weight follows directly by
putting \(y=e^{-\theta}\) and \(\epsilon=E-\Delta\). The leading edge
equation is
\(-4\Delta y^3+2\epsilon y+\ii\Gamma=0\); its physical root rotates from
\(\arg y=\pi/2\) below the edge to \(\arg y=0\) above it. Since
\(S/Q\simeq\tfrac12\Imag(y^{-2})\) and
\(\dd\epsilon=(4\Delta y+\ii\Gamma/2y^2)\dd y\), integration across the
layer gives
\(\Imag[2\Delta\ln y-\ii\Gamma/(12y^3)]_{-\infty}^{+\infty}
=-\pi\Delta\). On the positive-energy axis the resulting edge
contribution therefore has the distributional limit
\begin{equation}
  \frac{S(E;Q)}{Q}
  \xrightarrow[Q\to0]{\mathcal D}
  -\pi\Delta\,\delta(E-\Delta).
  \label{eq:S_edge_distribution}
\end{equation}
Consequently, for a smooth weight \(w\),
\begin{equation}
  \int_0^\infty S(E;Q)w(E)\,\dd E
  =
  -\pi\Delta Q\,w(\Delta)+o(Q),
  \label{eq:S_edge_weight}
\end{equation}
unless \(w(\Delta)=0\).
It enters the coupled dirty-limit kinetic equations~\cite{belzig1999} as
\begin{equation}
  \mathbf j_L=-\DN\big(\mathcal D_L\bnabla\fL+S\,\fT\big),\qquad
  \mathbf j_T=-\DN\big(\mathcal D_T\bnabla\fT+S\,\fL\big),
  \label{eq:coupled_LT}
\end{equation}
with \(\mathcal D_L=\tfrac14\Tr(1-\hatg^R\hatg^A)\) and
\(\mathcal D_T=\tfrac14\Tr(1-\hatg^R\tau_3\hatg^A\tau_3)\) as in
\cref{eq:DL_DT_def} (reducing to \(1\) and \(\DOS^2\)
in the BCS limit), so a phase gradient mixes the energy (\(\fL\)) and charge
(\(\fT\)) modes. On the ideal bulk BCS background the mixing is
\(O(Q^3)\) at each fixed energy outside the edge layer, but an
energy-integrated observable with nonzero weight at the edge has an
\(O(Q)\) onset by \cref{eq:S_edge_weight}. Thus the pointwise outer
scaling cannot be promoted to a uniform weak-mixing statement. On a
background whose zero-current spectrum is already complex (proximity,
depairing of other origin, or sub-gap energies in such broadened
spectra), \(S(E)=2Q\,\Imag\sinh^2\theta_0(E)+O(Q^3)\) is already
pointwise first order in \(Q\). The frozen-spectrum cancellation, the
outer expansion, and the edge sum rule have been verified by computer
algebra and physical-root continuation.
...
complementary generalization keeps the order parameter \emph{real} but lets the
...
\cref{eq:flux_dressings_BCS}. The A1 continuity and coefficient-placement
structure generalize to this setting: the
energy-channel coefficient \(\mathcal D_L\) is promoted from its on/off
BCS form to a smooth, manifestly real function of the complex
...
\(\hatg^R\), as in main-text~\cref{M-eq:bcs_advanced_matrix},
\begin{equation}
...
\begin{equation}
  \mathcal D_L=\tfrac14\Tr\!\big[\openone-\hatg^R\hatg^A\big]=\cos^{2}\theta'',
  \qquad
  \mathcal D_T=\tfrac14\Tr\!\big[\openone-\hatg^R\tau_3\hatg^A\tau_3\big]=\cosh^{2}\theta',
...
\begin{equation}
  \mathcal D_L\,\mathcal D_T=\DOS^{2}.
  \label{eq:DLDT_identity}
\end{equation}
The longitudinal (energy) channel carries \(\mathcal D_L=\cos^{2}\theta''\), reducing to
...
carries \(\mathcal D_T=\cosh^{2}\theta'\to\DOS^{2}\) on the real gap and remains
...
The energy-resolved operator A1 therefore generalizes to
\begin{equation}
...
\emph{adiabatic} reduction: it is exact to first Wigner (Moyal) order, the
instantaneous BCS propagator solving \([L_0,\hatg^R]_\star=0\) at
\(O(\hbar^0)\) and \(O(\hbar^1)\). The last beyond-assumption thread is the
leading nonadiabatic correction, obtained from the second-order Moyal
expansion on the \((E,t)\) phase space.

\paragraph{Spectrum.} Write the instantaneous propagator and its orthogonal
partner as
\begin{equation}
  \hat g_0=\frac{E}{W}\tau_3+\ii\frac{\Delta}{W}\tau_2,
  \qquad
  \hat m_0=\frac{\Delta}{W}\tau_3+\ii\frac{E}{W}\tau_2,
  \qquad W=\sqrt{E^2-\Delta^2}.
  \label{eq:nonad_basis}
\end{equation}
Carrying the instantaneous result to second order gives the off-diagonal
residual
\begin{equation}
  [L_0,\hat g_0]_\star
...
\end{equation}
On the real above-gap branch used here, it is canceled by the normalized
spectral correction
\begin{equation}
  \hat g^R=\hat g_0+\hbar^2\hat d+O(\hbar^3),
  \qquad
  \hat g^A=-\hat g_0-\hbar^2\hat d+O(\hbar^3),
  \qquad
  \hat d=\alpha\hat g_0+\beta\hat m_0,
  \label{eq:nonad_spectral_correction}
\end{equation}
where
\begin{equation}
  \alpha=\frac{W^2\dot\Delta^2-2E^2\Delta\ddot\Delta}{8W^6},
  \qquad
  \beta=-\frac{3E\Delta^2\ddot\Delta}{8W^6}.
  \label{eq:nonad_coefficients}
\end{equation}
Thus the physical advanced correction is \(\hat d^A=-\hat d^R=-\hat d\).
The corrected retarded and advanced propagators obey both their spectral
equations and \(\hat g^{R,A}\star\hat g^{R,A}=\openone\) through
\(O(\hbar^2)\). In particular, with
\begin{equation}
  d_3=\frac{E\alpha+\Delta\beta}{W},
\end{equation}
the real above-gap density of states becomes
\(\DOS+\hbar^2d_3+O(\hbar^3)\) (and generally acquires
\(\hbar^2\Real d_3\)). This is a genuine spectral correction. It does not,
by itself, determine the nonadiabatic spatial transport coefficients; doing
so requires a Moyal-corrected derivation of the spatial current.

\paragraph{Distribution.} With
\(\hat h=\fL\openone+\fT\tau_3\), the normalized Keldysh ansatz must use the
corrected spectral propagators:
\begin{align}
  \hat g^K
  &=\hat g^R\star\hat h-\hat h\star\hat g^A \notag\\
  &=\hat g_0\star\hat h+\hat h\star\hat g_0
    +\hbar^2(\hat d\hat h+\hat h\hat d)+O(\hbar^3),
  \label{eq:nonad_keldysh_correction}\\
  \hat d\hat h+\hat h\hat d
  &=2\fL\hat d+2\fT d_3\openone.
  \label{eq:nonad_keldysh_modes}
\end{align}
This form obeys
\(\hat g^R\star\hat g^K+\hat g^K\star\hat g^A=0\) through
\(O(\hbar^2)\). The time-dependent kinetic operator
\(\ii[L_0,\hat g^K]_\star\) keeps the energy and charge modes decoupled:
through \(O(\hbar^2)\) the plain and \(\tau_3\) projections retain the
adiabatic scalar result, while the off-diagonal projection has no \(\fT\)
source. In particular, the coherence-channel projection vanishes at first
order,
\begin{equation}
  \Tr\big[\tau_1\,\ii[L_0,\hat g^K]_\star\big]\Big|_{O(\hbar)}=0,
  \label{eq:nonad_coherence}
\end{equation}
and its complete second-order residual is
\begin{align}
  \left.\Tr\big[\tau_1\,\ii[L_0,\hat g^K]_\star\big]\right|_{O(\hbar^2)}
  ={}&-\frac{2\ii\hbar^2}{W^3}\Big[
    (E^2+\Delta^2)\dot\Delta\,\partial_E\partial_t\fL \notag\\
  &+E\Delta\big(\partial_t^2\fL
                 +\dot\Delta^2\partial_E^2\fL\big)
    +\Delta^2\ddot\Delta\,\partial_E\fL\Big].
  \label{eq:nonad_coherence_source}
\end{align}
The \(O(\hbar^2)\) term from
\(\hbar^2(\hat d\hat h+\hat h\hat d)\) is essential: it cancels the
same-order Keldysh term that would otherwise leave a false
\(3\ii\hbar^2 E\Delta^2\ddot\Delta\fL/W^5\) contribution. For every sufficiently smooth
shell-slaved distribution \(\fL=G(W)\), all terms in
\cref{eq:nonad_coherence_source} cancel identically; \(\fL=1\) is the
simplest check. A generic independently driven \(\fL(E,t)\), by contrast,
does leave a coherence-channel source. Even for a static gap it contains
\(-2\ii\hbar^2E\Delta\,\partial_t^2\fL/W^3\). No \(\fT\) source is generated.

\paragraph{Summary.} The spectral propagator and density of states acquire
the nonzero correction \cref{eq:nonad_spectral_correction}, while the
collisionless coherence-channel source
\cref{eq:nonad_coherence_source} cancels exactly for a shell-slaved
distribution. For a general independent drive the latter is a residual of
the collisionless kinetic equation, not yet a branch-coherence amplitude;
its amplitude and relaxation require an explicit collision or dephasing
closure. The scalar energy and charge projections remain unmixed at this
order, but no claim about the full nonadiabatic \(\mathcal D_L\) or
\(\mathcal D_T\) follows without the corresponding spatial-current
calculation.
These are pointwise statements away from the ideal gap edge: the correction
carries inverse powers of \(W=\sqrt{E^2-\Delta^2}\)
[\cref{eq:nonad_spectral_correction,eq:nonad_coefficients}], so the adiabatic
ordering requires the spectral angle to change slowly on the local
branch-splitting scale,
\(\hbar|\partial_t\theta|/W\sim\hbar E|\dot\Delta|/W^3\ll1\) (with the
analogous \(\ddot\Delta\) condition,
\(\hbar^2E^2\Delta|\ddot\Delta|/W^6\ll1\)); the relative density-of-states
correction \(\hbar^2d_3/\DOS\) is
\((\hbar\dot\Delta/W^2)^2/8+O(\ddot\Delta)\). This is the condition for the
fixed-energy instantaneous-BCS spectral functions themselves, and it is
distinct from, and on shell stronger by \(\DOS^4\) than, the
branch-adiabaticity parameter \(\hbar|\xi\dot\Delta|/E_\xi^3\) of
\cref{eq:branch_coherence_estimate}, which is bounded and vanishes at the
gap edge. In terms of the single small parameter
\(\hbar|\dot\Delta|/\Delta^2\) the expansion fails inside an edge layer
\((E-\Delta)/\Delta\lesssim\tfrac12(\hbar|\dot\Delta|/\Delta^2)^{2/3}\).
Sufficiently close to the edge the
expansion must be regularized by lifetime broadening, self-consistency, or
an explicit edge treatment. The spectral equations and star normalization,
the energy/charge decoupling, \cref{eq:nonad_coherence_source}, and its exact
shell-slaved cancellation were verified symbolically for generic
\(\Delta(t)\).

\subsection{Remark: lifetime-broadened (Dynes) spectra}
\label{app:dynes_remark}

The ideal-BCS trace simplifications use the real-coefficient identity
\(\DOS^2-N_2^2=1\). For a Dynes continuation, write
\[
z=E+\ii\Gamma,\qquad
g_\Gamma^R=\frac{z}{\sqrt{z^2-\Delta^2}},\qquad
f_\Gamma^R=\frac{\Delta}{\sqrt{z^2-\Delta^2}},
\]
with the causal advanced partner. Matrix normalization
\((g_\Gamma^R)^2-(f_\Gamma^R)^2=1\) survives, whereas the corresponding
identity among their real parts does not. The Dynes spectrum is a particular
complex spectral angle, so the general closed forms
\(\mathcal D_L=\cos^2(\Imag\theta)\) and
\(\mathcal D_T=\cosh^2(\Real\theta)\) apply, and the exact product
\(\mathcal D_L\mathcal D_T=\DOS^2\) survives. Thus the ideal individual
relations \(\mathcal D_L=1\) and \(\mathcal D_T=\DOS^2\) fail; only trace
quantities for which no general-spectrum form has been derived require
separate re-evaluation.

Finite \(\Gamma\) also changes the boundary physics. Because
\(\mathcal D_L=\DOS^2/\mathcal D_T>0\) wherever the broadened DOS is nonzero,
the ideal hard sub-gap zero-flux face becomes leaky. Deep below the gap,
\(\mathcal D_L(0)=\Gamma^2/(\Delta^2+\Gamma^2)\), while the leakage is order
unity within an \(O(\Gamma)\) edge layer. Broadening still regularizes the
gap-edge divergence and the matched-gap singularity of the
Kupriyanov--Lukichev charge-channel interface weight.
...
identities behind it; \cref{sec:fT} shows that the gap drive does not
generate transverse imbalance in the symmetric sector;
\cref{sec:verdict} collects the interpretation; and
...
and the associated binary product $A\star B$ (\cref{eq:moyal_def}; the
same product is written $\circ$ in the Conventions).
...
(The base of this projection is the identity, not $P_+$
[\cref{eq:bdg_branch_projectors}] or $\tau_3$.)

\subsection{The exact target identity}
\label{sec:exact}

The advective (branch-Boltzmann) form and the matrix conservative form are
related by an exact identity. With $u_E\equiv(\Delta/E)\dot\Delta$ and
...
\begin{equation}
  ...
  \label{eq:Lpiece}
\end{equation}
proportional to the identity with \emph{no} off-diagonal residue
(expanding the derivatives, the excess over the advective form of
\cref{eq:exact} is $\fL(\partial_t\DOS+\dot\Delta\,\partial_EN_2)$, which
vanishes by DOS-continuity). Plain trace:
...
=\hbar\DN\bnabla^{2}\fL+\hbar J_1$ ($J_1$ as in \cref{eq:J1_def}),
reproducing \cref{eq:expected_conservative}:
...
\begin{equation}
  ...
  \label{eq:specR}
\end{equation}
the vanishing being \emph{identical} to BCS DOS-continuity (this is
\cref{eq:gR_star_DOS_continuity}, recovered here within the verification
chain). So the identity that
...
\subsection{The zero-transverse sector is invariant under the gap drive}
\label{sec:fT}

Could the linear-in-$\dot\Delta$ flow in the energy equation instead be
mediated by charge imbalance? Not in the charge-symmetric sector considered
here. First,
...
verification extends the statement through $O(\hbar^2)$. Second, the
assembled transverse equation \cref{eq:fT_kinetic_conservative} is
homogeneous in $\fT$: the gap drive advects and dilutes an existing
imbalance, while \(J_T[\fL,0,n]=0\) supplies no source. With zero initial and
boundary imbalance, \(\fT=0\) therefore remains an exact solution through
the retained order. There is no drive-generated
\(\fT=O(\dot\Delta)\) leg and hence no associated
\(O(\dot\Delta^2)\) feedback loop. Such a loop requires an explicit source
outside this sector, for example injected charge imbalance,
particle--hole-asymmetric drive or collisions, an electric field, or a
supercurrent/complex phase. The linear spectral-flow term is instead the
direct kinematic effect on \(\fL\) produced in \cref{eq:Lpiece}.

\subsection{Interpretation}
\label{sec:verdict}

The verification succeeds: the plain-trace projection produces the
conservative flow, \cref{eq:Lpiece}. Combined with \cref{eq:exact} and
...
matrix equation remains available but is not constructed here. Exact scalar
relabelling determines the corresponding scalar law away from $\xi=0$,
including its connection terms; \cref{sec:resolution} verifies only the
fixed-$E$ spatial matrix current.
...
assembled in \cref{sec:transverse_projection}; \cref{sec:fT} uses its
homogeneous structure together with the source condition
\(J_T[\fL,0,n]=0\). The trace identities
\cref{eq:Lpiece,eq:fTtrace,eq:specR,eq:specA} have been verified independently by
symbolic computer algebra (identities I1, I2/I2b, and I4 of the
verification scripts; I3 verifies the sandwich weight quoted in
\cref{sec:plain}; see the data and code availability statement of the
main text).

\subsection{Fixed-\texorpdfstring{$E$}{E} spatial-current check for a varying gap}
\label{sec:resolution}

With the time-dependent flow produced directly by the fixed-$E$ projection
(\cref{sec:plain}), the remaining calculation is the fixed-$E$ spatial
current for a varying gap. The calculation below proves, by direct Pauli
traces and symbolic algebra, that this current contains no
$\bnabla\Delta$ drift or cross-channel term at the retained order. It does
not construct the matrix current intrinsically in a moving-$\xi$ frame.

\paragraph{Spatially varying gap.} Within the fixed-$E$ representation, a
direct derivation must retain every spectral-gradient contribution for a
spatially varying gap $\Delta(\br)$. We specialize to one spatial dimension
$x$ for brevity; the conclusion is identical for the full gradient
$\bnabla$. Take the static, real $\Delta(x)$, leading-gradient
...
\paragraph{Interpretation.} For
$F(\xi,x)=\fL(E(\xi,x),x)$, the coordinate change gives
\[
  \partial_x\big|_E
  =
  \partial_x\big|_\xi
  -\frac{\Delta}{\xi}(\partial_x\Delta)\,\partial_\xi
  \equiv\mathcal C_x .
\]
Thus exact scalar relabelling sends the fixed-$E$ diffusion operator to
$\mathcal C_x(\mathcal C_xF)$; the connection acts on both the current and
its divergence. These terms are required by the coordinate change, not an
independent physical DOS-gradient drift. The calculation above establishes
the fixed-$E$ current with no cross-channel mixing; it does not demonstrate
an intrinsic moving-frame matrix cancellation. The moving-$\xi$ chart is
nonuniform at $\xi=0$, where its connection diverges.
...
adiabatic, charge-balanced, leading dirty-limit gradient order, and no phase
gradient or supercurrent), the fixed-$E$ result is closed by the two exact
identities
\begin{equation}
...
spectral-flow equivalence, while collisions transform by the same DOS
Jacobian. There is no room in these fixed-$E$ identities for the A2 density
$\DOS^2 f$ or for hidden longitudinal--transverse mixing. Exact scalar
relabelling determines the moving-$\xi$ scalar law away from the gap edge;
no claim is made here about an intrinsic moving-frame matrix derivation,
higher-gradient truncations, or uniformity at $\xi=0$.
...
fixed-energy outer $O(Q^3)$ but energy-integrated $O(Q)$ on the ideal BCS
background, and pointwise $O(Q)$ on already-complex ones).
...
(iii)~The normalized nonadiabatic spectral correction and the collisionless
coherence-channel residual are obtained at $O(\hbar^2)$ in
\cref{app:nonadiabatic}; the Moyal-corrected spatial current and collision-level
physics (memory, drive sidebands, branch-coherence relaxation, or a gap
changing appreciably during a collision) remain open, together with
...
\cref{sec:program} [\cref{fig:bench_uniform,fig:bench_drift,fig:bench_interface}]. Every identity used
...
The reductions of the two main-text routes can be compared without changing
notation by
...
The second term of \cref{eq:Lpq_expanded} is a
density-of-states-gradient drift, and it has an exact first-moment
reading. On a static profile each member conserves the density
\(\DOS^{p}f\), and two integrations by parts (boundary fluxes
discarded) give, for the center of mass of that density,
\begin{equation}
  \frac{\dd\avg{\br}}{\dd t}
  =
  \avg{\,q\,\DN\,\DOS^{\,q-p-1}\,\bnabla_{\br}\DOS\,},
  \label{eq:Lpq_first_moment}
\end{equation}
the average taken over the conserved density itself. This moment is a
structural diagnostic of \(q\neq0\), but it is a different physical
quantity for each member of the family: for the \(p=0\) placements it is
the first moment of the bare occupation, which is not the density of
anything. The quantity common to all members is the quasiparticle
density \(\DOS f\), whose energy integral is \(n_{\qp}\)
(\cref{sec:self_consistent_feedback}). For a readout density
\(\DOS^{s}f\) the same two integrations by parts give
\begin{equation}
  \begin{split}
  \frac{\dd\avg{\br}_{s}}{\dd t}
  &=
  (q+s-p)\,\DN\avg{\DOS^{\,q-p-1}\bnabla_{\br}\DOS}_{s}
  \\
  &\quad
  +(s-p)\,
  \frac{\int(\br-\avg{\br}_{s})\,\DOS^{\,s-p-1}\,
        (\bnabla_{\br}\DOS\cdot\mathbf j)\,\dd^{d}r}
       {\int\DOS^{s}f\,\dd^{d}r},
  \end{split}
  \label{eq:readout_first_moment}
\end{equation}
with \(\mathbf j=-\DN\DOS^{q}\bnabla_{\br}f\) the flux of the conserved
density and \(\avg{\cdot}_{s}\) the average over \(\DOS^{s}f\); \(s=p\)
recovers \cref{eq:Lpq_first_moment}. For a packet narrow on the scale
over which \(\bnabla\DOS\) varies the shape term reduces to
\((s-p)\DN\avg{\DOS^{\,q-p-1}\bnabla_{\br}\DOS}_{s}\), so that
\begin{equation}
  \frac{\dd\avg{\br}_{s}}{\dd t}
  \simeq
  \bigl[q+2(s-p)\bigr]\,\DN\,\DOS^{\,q-p-1}\,\bnabla_{\br}\DOS .
  \label{eq:readout_drift_narrow}
\end{equation}
On the quasiparticle density (\(s=1\)) the prefactor is \(q+2(1-p)\).
For A1, \((p,q)=(1,0)\), it vanishes, and both terms of
\cref{eq:readout_first_moment} vanish with it at every packet width:
nothing moves on any static profile, because the microscopic operator
responds to gap inhomogeneity only through its conserved-density weight
and the subgap blocking, its undressed current integrating to a boundary
term. The prefactor also vanishes for A2, \((2,2)\), and for the
constant-\(\tau\) placement B, \((0,-2)\), whose flux drift and
conserved-weight response cancel in the quasiparticle density at leading
order, leaving only the finite-width remainder of the shape term in
\cref{eq:readout_first_moment}. The legacy placement C,
\((p,q)=(0,-1)\), moves its own conserved moment, the bare occupation, at
\begin{equation}
  \bv_{C}^{(0)}
  =
  \bnabla_{\br}\frac{\DN}{\DOS}
  =
  -\frac{\DN}{\DOS}\,\bnabla_{\br}\ln\DOS
  =
  -\DN\,\frac{\Delta\,\bnabla_{\br}\Delta}{E\sqrt{E^{2}-\Delta^{2}}},
  \label{eq:legacy_drift_velocity}
\end{equation}
and its quasiparticle density at the equal and opposite
\begin{equation}
  \bv_{C}
  =
  +\DN\,\DOS^{-2}\,\bnabla_{\br}\DOS
  =
  +\DN\,\frac{\Delta\,\bnabla_{\br}\Delta}{E\sqrt{E^{2}-\Delta^{2}}}.
  \label{eq:legacy_density_drift}
\end{equation}
At fixed energy \(\DOS\) increases with the local gap, so the
quasiparticle-density drift points up the gap gradient and diverges
toward the local gap edge, the energies at which quasiparticle
populations concentrate. On any gap well, whether an engineered trap or
the suppression a localized population digs for itself through the gap
equation (\cref{sec:self_consistent_feedback}), the legacy placement
therefore drives the quasiparticle density of a passive fixed-energy
distribution out of the well, while A1 leaves it in place; in the
narrow-packet law the difference
\(\bv_{C}-\bv_{\mathrm{A1}}=\DN\DOS^{-2}\bnabla_{\br}\DOS\) is
independent of the readout weight \(s\).
The drift is spurious in a precise sense: it is produced by the
placement of the spectral weight, not by the microscopics; neither
route (main-text~\cref{M-sec:scalar_diffusion,M-sec:usadel_route}) yields it. Nor is
the artifact a statement about physical trapping: gap-edge blocking and
inelastic relaxation into gap-suppressed regions survive in A1, and
what A1 lacks is solely the fixed-energy advective response to
\(\bnabla\DOS\). The benchmarks of \cref{sec:program} measure the
quasiparticle-density drift of \cref{eq:readout_first_moment} directly,
and \cref{sec:self_consistent_feedback} constructs a
local P\'eclet number comparing its magnitude with ordinary diffusion.

The family also organizes conservation-law bookkeeping. If the conserved
shell density is \(\mathcal W(E,\br)\,f\), transported by the weighted
occupation-gradient current
\(\mathbf j=-\mathcal W\,\DE\,\bnabla_{\br}f\) [the structure of
main-text~\cref{M-eq:generic_energy_current}], the occupation obeys
\(\partial_tf=\mathcal W^{-1}\bnabla_{\br}\cdot[\mathcal W\,\DE\,\bnabla_{\br}f]+\cdots\);
with \(\mathcal W=\DOS\) and a mobility \(\DE=\DN\DOS^{\,r}\) this is the
member \((p,q)=(1,r+1)\). The continuity-skeleton row of
\cref{tab:operator_taxonomy} records the skeleton; the mobility choice
assigns the label, and \(r=-1\) is A1.

\Cref{fig:routes_roadmap} summarizes how the candidate operators descend
from the common quasiclassical parent. The antipodal branch relabelling
commutes with the bare angular average
(\cref{sec:projection_vs_averaging}); the two routes instead differ in how
they retain and eliminate the current-carrying first harmonic. Their scalar
end points coincide on A1, and \cref{tab:operator_taxonomy} collects that
operator together with the legacy and diagnostic placements.
...
\begin{figure*}[t]
  ...
  candidate diffusion operators of \cref{tab:operator_taxonomy}. The bare
  angular average is invariant under the antipodal branch relabelling
  (\cref{sec:projection_vs_averaging}), which shuffles odd angular harmonics
  between the longitudinal and transverse representations. What differs is
  whether the current-carrying first harmonic is closed as a scalar branch
  moment or as a matrix moment before the Nambu trace. The scalar end points
  nevertheless coincide: the
...
main-text~\cref{M-sec:tdep_spectral_flow}), and equally the branch-projected moment
reduction with the BRT-dressed kernel
[main-text~\cref{M-eq:sc_scalar_diffusion,M-eq:sc_tau_closure}]. The \((0,\cdot)\)
...
\(\mathcal{D}_T=\DOS^2\) of main-text~\cref{M-eq:DL_DT_BCS_bulk} to the occupation
...
[main-text~\cref{M-eq:sc_scalar_diffusion}] and the BRT-dressed kernel
[main-text~\cref{M-eq:sc_tau_closure}] are kept: conserved density \(\DOS f\), undressed flux, uniform-gap rate
\(\DN/\DOS(E)\), with the time-dependent spectral-flow term produced
directly by the fixed-\(E\) projection (main-text~\cref{M-sec:tdep_spectral_flow}).
...
Appendix~A, Eq.~(A1), of the gap-engineered-trap analysis of
...
mode [main-text~\cref{M-eq:DL_DT_BCS_bulk}]), and A2 further replaces the time
...
The route comparison reduces the problem to a small set of explicit
checks and benchmarks. Two pieces are fixed by standard theory: the
dirty-limit projection selects A1 (main-text~\cref{M-sec:usadel_route}),
and the BRT coherence-factor cancellation fixes the branch-route
kernel, whose ordered reduction
[main-text~\cref{M-eq:sc_scalar_diffusion,M-eq:sc_tau_closure}] lands
on the same A1. This section states the regime boundaries of the
shared reduction, exhibits the gap-feedback response that separates
the candidate operators, and then benchmarks the resulting operators
numerically; the companion interface reduction of the matrix boundary
condition is derived in main-text~\cref{M-sec:conserved_currents}.
\subsection{Projection versus angular averaging}
\label{sec:projection_vs_averaging}

The Usadel route averages over Fermi-surface (momentum) directions before
extracting a scalar quasiparticle occupation. The scalar Boltzmann route extracts the
branch occupation first and closes its angular moments afterward. The
antipodal relabelling involved in the branch projection does \emph{not}
obstruct the bare average. For any integrable angular function \(X\), define
\begin{equation}
  R_sX(\hat{\bp})\equiv X(s\hat{\bp}),
  \qquad
  \avg{R_sX}_{\hat{\bp}}=\avg{X}_{\hat{\bk}},
  \qquad
  \hat{\bk}=s\hat{\bp},\quad s=\pm1 .
  \label{eq:antipodal_average_identity}
\end{equation}
The equality is just the measure-preserving change of variable
\(\hat{\bk}=s\hat{\bp}\). The representation dependence instead resides
in retaining, solving, and eliminating the current-carrying first harmonic.
The branch-to-trajectory map flips its odd angular harmonics
[main-text~\cref{M-eq:intro_parity_dictionary}], so the branch route's
current-carrying first harmonic is the anisotropic
...
as the hidden-harmonic map main-text~\cref{M-eq:sc_hidden_harmonic} and the
closed operator main-text~\cref{M-eq:sc_LD_closed} show explicitly: the scalar end points coincide on A1
(\cref{fig:routes_roadmap}). Thus the relabelling changes the
longitudinal/transverse representation of the first harmonic, not the full
angular average or the resulting diffusion law. Genuinely different kinetics
appears only outside the shared assumptions: ballistic
...
in \cref{app:supercurrent,app:proximity,app:nonadiabatic}.

\subsection{Self-consistent gap feedback}
\label{sec:self_consistent_feedback}

Three of the four benchmark problems defined below treat \(\Delta(\br)\) as prescribed. In a device the
...
equation [cf.~main-text~\cref{M-eq:intro_three_gap}],
\(1=\lambda\int_0^{\omega_D}\dd\xi\,[1-2f(E)]/E\), by its
zero-temperature counterpart (\(f=0\); \(\Delta_0\) the zero-temperature
equilibrium gap) cancels the cutoff (\(\omega_D\gg\Delta\)) and
...
\cref{eq:Lpq_expanded} converts this into the first-moment drift of
\cref{eq:Lpq_first_moment}, with the sign of \(q\): divergent
at the edge for \(q-p>-2\) and finite precisely at the marginal exponent
\(q-p=-2\) of the legacy placement B; on the quasiparticle density the
prefactor becomes \(q+2(1-p)\) [\cref{eq:readout_drift_narrow}], which
also vanishes for B and A2; the
dirty-limit operator A1 (\(q=0\)) has no drift response at any amplitude.

For the legacy placement C, \((p,q)=(0,-1)\), that drift has the
fixed-energy magnitude \(|\bv_C|\) of
\cref{eq:legacy_drift_velocity,eq:legacy_density_drift} (directed out of
the well on the quasiparticle density). A
compact local diagnostic compares it with diffusion over a length
\(L_{\Delta}\), where
\(|\delta\Delta|\simeq L_{\Delta}|\partial_x\Delta|\):
\begin{equation}
  \mathrm{Pe}_E
  \equiv\frac{|\bv_C|L_{\Delta}}{\DE}
  =\frac{\Delta|\delta\Delta|}{E^2-\Delta^2},
  \qquad \DE=\frac{\DN}{\DOS}.
  \label{eq:legacy_local_peclet}
\end{equation}
At \(E/\Delta=(1.05,1.10,1.30)\), the corresponding triples are
\begin{align*}
  \mathrm{Pe}_E&=(9.76,4.76,1.45)\times10^{-5}
    &&\bigl(|\delta\Delta|/\Delta=10^{-5}\bigr),\\
  \mathrm{Pe}_E&=(9.76,4.76,1.45)\times10^{-3}
    &&\bigl(|\delta\Delta|/\Delta=10^{-3}\bigr),\\
  \mathrm{Pe}_E&=(1.95,0.952,0.290)
    &&\bigl(|\delta\Delta|/\Delta=0.2\bigr).
\end{align*}
This P\'eclet number is a local fixed-energy operator diagnostic, not a
device observable or a predicted device error. It is nonuniform at the
ideal BCS gap edge; translating it into an error in a density, lifetime,
or readout requires a broadened spectrum, an energy distribution, a
collision kernel, and the full boundary-value problem.
...
the first Moyal correction to \(g_0^K\) is sourced only by \(\fT\), while
the leading candidate \(\hbar\,\dot\Delta\,\partial_x\Delta\) cross term, which the outer Moyal brackets of \(\checkg_0\,\widetilde\bnabla\checkg_0\)
do generate in the \(\fL\) sector, separately traces to zero in the energy mode (verified symbolically for generic
\(\Delta(x,t)\) alongside the identities of main-text~\cref{M-sec:tdep_spectral_flow};
script \texttt{verify\_tdep\_inhomogeneous.py} in the verification
repository cited under Data and code availability in the main text). Self-consistency changes
the \emph{background} A1 runs on, not the operator. Second, feedback
makes the inhomogeneous-gap regime self-generating: a localized
population suppresses the gap, so the structure that separates the
candidate operators can be engaged without an externally imposed
profile. The fourth benchmark below uses a distinct passive probe to
read out transport on this population-generated landscape.

\subsection{Benchmark problems}

Four benchmark problems separate the models cleanly:
\begin{enumerate}
  \item a uniform-gap relaxation-rate test, where the microscopic operator A1
  ...
  placement C carries the spurious drift of its \(q=-1\) flux, which
  moves its quasiparticle density up the gap gradient at
  \(\DN\DOS^{-2}\partial_x\DOS\) [\cref{eq:legacy_density_drift}], the
  \((1,2)\) diagnostic moves it the same way at, asymptotically
  (\(E\gg\Delta\)), twice that rate, and A2 and B carry no
  quasiparticle-density drift at leading order in the gradient;
  ...
  \item a self-consistent feedback problem, in which a maintained
  quasiparticle population generates a gap well through the gap closure
  \cref{eq:gap_feedback_closure}, and a separate probe packet, excluded
  from the gap closure and therefore passive, reads out each operator's
  response on the well's flank.
\end{enumerate}
...
\cref{sec:self_consistent_feedback}: it asks how each operator transports
a passive probe on a gap landscape generated by a distinct quasiparticle
population.
...
(\cref{fig:bench_uniform}); the ramp drift velocities (measured on the
center of mass of the quasiparticle density \(\DOS f\), the same readout
for every operator) trace the exact initial rate of
\cref{eq:readout_first_moment}, asymptotically
\(\DN\,[q+2(1-p)]\,\DOS^{\,q-p-1}\partial_x\DOS\): A1 is driftless, C and
the \((1,2)\) diagnostic move up the gap gradient, and A2 and B carry no
net drift
...
\begin{figure}[t]
  ...
  energy by energy, of a Gaussian packet
  \(\exp[-(x-x_0)^2/\sigma^2]\) (\(\sigma=8~\mu\mathrm{m}\)) on
  a static linear ramp \(\Delta(x)\) rising from \(\Delta_0\) to
  \(1.6\,\Delta_0\) across the strip, with no collisions. Lines are the
  exact initial rate of \cref{eq:readout_first_moment} on the initial
  packet, which on this ramp agrees with the narrow-packet form
  \(v=\DN\,[q+2(1-p)]\,\DOS^{\,q-p-1}\,\partial_x\DOS\) to about 2\%;
  markers are measured over \(4\)~ns. The tracked moment is the center
  of mass of the quasiparticle density \(\DOS f\), the same physical
  quantity for every operator; each operator's own conserved-density
  moment \(\DOS^{p}f\) would instead isolate the flux drift
  \(\DN\,q\,\DOS^{\,q-p-1}\partial_x\DOS\) of \cref{eq:Lpq_first_moment},
  which for the \(p=0\) legacy placements points the opposite way. A1
  shows no DOS-gradient drift; the legacy placement C (\(q=-1\)) and the
  \((1,2)\) diagnostic drift up the gap gradient, with magnitudes set
  asymptotically by \(q+2(1-p)=1\) and \(2\); A2 and B
  (\(q+2(1-p)=0\)) carry no net drift of \(\DOS f\). The prescribed 60\% ramp is deliberately
  large to discriminate the operators; it is not intended as a typical
...
\begin{figure}[t]
  ...
  \caption{Interface benchmark: a same-material two-gap Al strip
  (\(1.6\,\Delta_0\) for
  \(x<50~\mu\mathrm{m}\), \(\Delta_0\) beyond) joined by a
  Kupriyanov--Lukichev face carrying the coherence-factor weight of
  main-text~\cref{M-eq:scalar_BC_energy} (interface conductance in diffusion units,
  \(g_N=G_N/2e^2N_0=0.1~\mu\mathrm{m}/\mathrm{ns}\); no side label is needed
  because \(N_0\) is the same on both sides), shown at a fixed energy
  above both gap edges. (a)~Driven steady state with a fixed-\(f\)
  source at the left edge and an absorbing (\(f=0\)) right edge: each
  operator shown carries equal currents on the
  ...
  \(\sim2\times10^{-3}\) at the drive energy. This idealized K-L face
  tests scalar current matching; it is not a transfer law for the
  coherence-scale proximity crossover of Ref.~\onlinecite{riwar2019}.}
...
out at \(E=1.08\,\Delta_0\) on its quasiparticle density \(\DOS f\)). In
the quasi-static well the initial center-of-mass rate of the probe's
quasiparticle density follows the exact law
\cref{eq:readout_first_moment} evaluated on the self-generated profile:
the legacy placement C drives the passive probe \emph{out of} the
population-generated well (\(+0.024~\mu\mathrm{m}/\mathrm{ns}\) fitted
over the first nanosecond against an initial rate of \(+0.027\); the
shortfall is the packet spreading during the fit window), the \((1,2)\)
diagnostic expels it more strongly (\(+0.325\) against \(+0.383\)), A2
and B show only the finite-width remainder of
\cref{eq:readout_first_moment} set by the well's curvature (\(+0.025\)
and \(-0.003\); their leading-order drift is zero), and A1 produces no
fixed-energy DOS-gradient motion: zero to round-off at every time in the
quasi-static calculation, since its undressed face flux telescopes across
the well. Beyond the first few nanoseconds the packets have spread to the
width of the well and the centroid of \(\DOS f\) mixes drift with the
spatial variation of the readout weight itself, so the long-time curves
of \cref{fig:bench_feedback}(b) are no longer drift velocities; A2's late
rise and C's turnover are this finite-width effect. With fully dynamic
feedback (the gap tracking the well-generating population every step)
the sign pattern of the initial rates survives (C \(+0.014\), the
\((1,2)\) diagnostic \(+0.24\), A2 and B within \(0.007\) of zero); the
bookkeeping drift of the conserved total from the energy-space
spectral-flow advection (which the fixed-\(E\) benchmark discretization
omits) is measured rather than hidden, at the percent level
(\(2\)--\(5\%\)) for the \(p\geq1\) operators over the \(20\)-ns run and
zero to round-off for \(p=0\) (A1's residual center-of-mass motion under
dynamic feedback, \(-0.009~\mu\mathrm{m}/\mathrm{ns}\), is this
bookkeeping effect, comparable to the legacy drift at this well depth,
not a DOS-gradient response).
A fixed-energy DOS-gradient drift of a passive tracer's quasiparticle
density, directed out of a population-generated gap well, is therefore a
structural artifact of the legacy placement C (and of the
transverse-dressed diagnostic); the microscopic operator (by either
route) has no such drift.
...
\begin{figure}[t]
  ...
  probe packet on the flank. Only the well-generating population enters
  the gap closure; the separate probe is excluded and is therefore
  passive. (b)~Center-of-mass shift of the probe's quasiparticle density
  \(\DOS f\) at
  \(E=1.08\,\Delta_0\) in the quasi-static well: the legacy placement C
  and the \((1,2)\) diagnostic A1\('\) (legend: A1P) move away from the
  well, the microscopic A1 stays exactly put at all times, and A2 and B,
  which have no leading-order drift, show the finite-width remainder of
  \cref{eq:readout_first_moment}, which grows as the packets spread to
  the width of the well (A2's late rise, C's turnover); beyond the first
  few nanoseconds the curves are no longer drift velocities.
  (c)~Drift velocity at the probe versus well depth, fitted over the
  first nanosecond (markers) against the exact initial rate of
  \cref{eq:readout_first_moment} (lines): the response scales with the
  feedback strength with the sign of \(q+2(1-p)\), the fitted means sit
  below the initial rates by the packet spreading during the fit window,
  and A1's vanishing drift is parameter-free. The 5\%
  well is deliberately exaggerated for discrimination, and the
  passive-probe drift is not a statement about the net spreading of the
  well-generating population.}
