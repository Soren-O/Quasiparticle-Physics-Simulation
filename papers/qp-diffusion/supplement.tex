...
Fermi-surface direction \(\hat\bp\) (a unit vector; the main text's
trajectory/group-velocity direction), spatial
...
(\cref{sec:slaving}) produce a real coefficient. Its relation to the
main-text starting display is exact and does not require reversing or
conjugating the Wigner kernel. Define the antipodally relabelled functions
\[
  \bar{\check g}(E,\hat{\bp})\equiv\check g(E,-\hat{\bp}),
  \qquad
  \bar{\check\Sigma}(E,\hat{\bp})
  \equiv\check\Sigma(E,-\hat{\bp}).
\]
Writing the main-text equation as \(V+\ii C=0\), evaluation at
\(-\hat{\bp}\) gives \(-V[\bar{\check g}]+\ii C[\bar{\check g}]=0\).
Multiplication by \(-\ii\) therefore gives
\(\ii V[\bar{\check g}]+C[\bar{\check g}]=0\), which is
\cref{eq:keldysh_eilenberger}. The Wigner product and energy variable are
unchanged. The angularly averaged Born self-energy retains its form under
this relabelling, and the resulting matrix Keldysh--Usadel equation
...
The Usadel route averages over trajectory (Fermi-surface) directions before
