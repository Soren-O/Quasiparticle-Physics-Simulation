...
\end{equation}
the average taken over the conserved density itself. A narrow packet
held at fixed energy therefore drifts at the local velocity inside the
brackets. For \(q=0\) nothing moves on any static profile: the
microscopic operator A1 responds to gap inhomogeneity only through its
conserved-density weight and the subgap blocking, its undressed
current integrating to a boundary term. The legacy placement C,
\((p,q)=(0,-1)\), instead drifts at
\begin{equation}
  \bv_{C}
  =
...
\end{equation}
At fixed energy \(\DOS\) increases with the local gap, so this
velocity points down the gap gradient and diverges toward the local
gap edge, the energies at which quasiparticle populations
concentrate. On any gap well, whether an engineered trap or the
suppression a localized population digs for itself through the gap
equation (\cref{sec:self_consistent_feedback}), the legacy placement
therefore drives a passive fixed-energy distribution into the well.
...
the artifact a proxy for physical trapping: gap-edge blocking and
...
drift spectrum of \cref{eq:Lpq_first_moment} directly and construct a
local P\'eclet number comparing it with ordinary diffusion.
...
For the legacy placement C, \((p,q)=(0,-1)\), that drift is the
fixed-energy velocity \(\bv_C\) of \cref{eq:legacy_drift_velocity}. A
...
\begin{figure}[t]
  ...
  additionally respond to the spatially varying conserved-density weight
  \(\DOS^{p}(x)\). A1 (\(q=0\)) shows no DOS-gradient drift; the \(q=2\)
  diagnostics drift up the gap gradient; the legacy placements C
  (\(q=-1\)) and B (\(q=-2\)) drift down, with magnitudes set
  asymptotically by \(q\). The prescribed 60\% ramp is deliberately
...
drive the passive probe \emph{into} the population-generated well (C and
B within a few percent of the prediction), the \(q=2\) diagnostics expel
it, and A1 produces no fixed-energy DOS-gradient motion: zero to
round-off in the quasi-static calculation, since its undressed face flux
telescopes across the well. With fully dynamic feedback (the gap tracking
the well-generating population every step) the same sign pattern survives; the
bookkeeping drift from the energy-space spectral-flow
advection (which the fixed-\(E\) benchmark discretization
omits) is measured rather than hidden, at the percent level for the
\(p\geq1\) operators over the \(20\)-ns run and zero for \(p=0\)
(A1's residual center-of-mass motion under dynamic feedback is this
bookkeeping effect, about a factor of \(3\) below the legacy drift at
this well depth, not a DOS-gradient response).
An inward passive-tracer response to a population-generated gap well is
therefore a structural artifact of the \(q<0\) legacy placements; the
microscopic operator (by either route) has no such fixed-energy
DOS-gradient drift.
...
\begin{figure}[t]
  ...
  passive. (b)~Probe center-of-mass drift at
  ...
  sign of \(q\), and A1's vanishing drift is parameter-free. The 5\%
  well is deliberately exaggerated for discrimination, and inward probe
  drift is not net compression of the well-generating population.}
