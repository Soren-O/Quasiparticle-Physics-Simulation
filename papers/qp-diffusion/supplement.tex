...
term the fixed-\(E\) reduction lacks. (Symbolic verification
[\cref{sec:resolution}] confirms the companion statement for
...
The trace simplifications of this appendix---and the channel dictionary
of main-text~\cref{M-tab:channel_dictionary} built on them---evaluate
the Nambu traces on the ideal BCS background, where the identity
\(\DOS^2-N_2^2=1\) holds among the \emph{real} spectral coefficients.
Inserting a finite lifetime broadening \(\ii\Gamma\) directly into the
spectral functions changes that identity---the matrix normalization
\(g^2-f^2=1\) survives the complex shift \(E\to E+\ii\Gamma\); it is the
identity among the real coefficients that fails---and therefore changes
the trace simplifications themselves rather than merely smoothing the
plotted coefficients. Where a finite \(\Gamma\) is physically required,
the Nambu traces above must be re-evaluated---generally numerically,
since the real-BCS simplification no longer applies---with the complex
spectral functions \(\DOS(E+\ii\Gamma)\), \(N_2(E+\ii\Gamma)\). The
broadening also regularizes the gap-edge structure of the spectral
coefficients (e.g.\ the divergence of the transverse dressing
\(\mathcal D_T=\DOS^2\)) and the matched-gap singularity of the
...
\paragraph{Interpretation.} The $\bnabla\Delta\,\partial_E\fL$ term a \emph{naive}
moving-$\xi$ current would generate via
$\bnabla_\xi F=\bnabla_E\fL+(\Delta/E)\bnabla\Delta\,\partial_E\fL$
(gradients at fixed $\xi$ and fixed $E$, respectively) is simply
