...
- **A1 is selected by BOTH routes**; taxonomy rows B \((0,-2)\)/C
  ...
  diagnostics only. A1 is \((p,q)=(1,0)\), so its DOS-gradient drift
  vanishes identically and parameter-free by both routes, on every readout
  weight. **REOPENED AND RE-SETTLED 2026-09-01 (Soren, option (a) of the
  physics-audit handoff):** the drift benchmarks read every operator on
  the SAME physical quantity, the quasiparticle density \(N_1 f\)
  (`READOUT_WEIGHT = 1` in `validation/diffusion_operators/__init__.py`).
  The exact first-moment law for a readout \(N_1^s f\) is
  eq:readout_first_moment; its narrow-packet form is
  \(v=D_N[q+2(s-p)]N_1^{q-p-1}\partial_xN_1\), so on \(N_1 f\) the
  prefactor is \(q+2(1-p)\): A1, A2 and B null, **C \(+1\) — the legacy
  placement drives a passive tracer's quasiparticle density OUT of a
  population-generated gap well**, the (1,2) diagnostic \(+2\). The old
  per-operator moment (each operator's own \(N_1^p f\),
  eq:Lpq_first_moment, prefactor \(q\), C inward) is a structural
  diagnostic of \(q\neq0\), not a statement about where quasiparticles
  go; \(v_C-v_{A1}=+D_N N_1^{-2}\partial_xN_1\) for every \(s\). **Do not
  reintroduce "inward" / "into the well" / "falls toward" for the legacy
  placements.** Measured 2026-09-01 (µm/ns, own moment → \(N_1 f\)): ramp
  near the gap edge C \(-0.023\to+0.024\), B \(-0.031\to0\), A2
  \(+0.072\to0\), A1P unchanged; self-dug 5% well at \(E=1.08\Delta_0\):
  C \(-0.033\to+0.024\). For \(s\neq p\) the narrow-packet law is only
  leading order in the gradient; the figures' analytic lines are the
  exact initial rate (`exact_initial_drift`), verified against a short
  CN step to 0.2%. The benchmark still does **not** establish reciprocal
  net self-focusing (or anti-focusing) of one coupled population — see
  "Open gates" below.
...
\(q=0\)); and — since the 2026-09-01 readout decision (see the A1 bullet
above) — the legacy placement C pushes a passive tracer's quasiparticle
density \(N_1 f\) *away from* a gap well generated by a *separate*
population, while B and A2 show no leading-order drift. (The
pre-2026-09-01 "toward the well" reading was each operator's own
\(N_1^p f\) moment, i.e. for C the bare occupation.)
...
concentration) overstates what's demonstrated; "passive-probe drift" or
"reduced broadening" does not. (The σ_x numbers above are widths of each
operator's own conserved density; they were not re-measured on
\(N_1 f\).)
...
probe is passive, and the probe drift is not a statement about the
well-generating population's spreading. **Don't
reintroduce "self-focusing" (or "anti-focusing") language** for the legacy operators unless
