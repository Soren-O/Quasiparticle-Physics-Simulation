...
The unified :class:`qpsim.backends.spatial.SpatialBackend` solves the strip as
the degenerate case of a ``(1, N)`` mask, and every physical property here --
current continuity across a gap ramp, the Kupriyanov-Lukichev interface, the
A1/A2/C closure separations -- holds cell for cell.

Mesh note: these tests mesh with ``x = np.linspace(0.0, 100.0, NX)``, so the
pitch is ``100/(NX - 1)``, NOT ``100/NX``. The geometry carries that pitch and
places cells at ``(arange(NX) + 0.5) * mesh_size``.
"""
...
def _uniform_state(
    *,
    ...
    """A thermal strip at a single scalar gap."""
...
class TestStripVaryingGap:
    ...
    def test_gap_profile_shape_validation(self) -> None:
        ...
        # silently broadcast or truncated.  The backend has no explicit
        # `gap_per_cell` shape check: the mismatch
        ...
        with pytest.raises((ValueError, IndexError), match=str(NX + 1)):
            SpatialBackend().apply_transport(bad, 1.0)
    ...
    def test_degenerate_and_descending_mesh_rejected(self) -> None:
        ...
        for bad_pitch in (0.0, -pitch):
            ...
            with pytest.raises(ValueError, match="positive and finite"):
                SpatialBackend().apply_transport(state, 1.0)
    ...
    def test_nonfinite_bath_is_rejected(self, bad: float) -> None:
        ...
        with pytest.raises(ValueError, match=r"(?i)T_bath|temperature"):
            SpatialBackend().apply_collisions(state, 1.0)
    ...
    def test_negative_interface_conductance_rejected(self, bad: float) -> None:
        ...
        # The non-finite cases are rejected downstream of the field:
        # -inf trips the Laplacian finiteness check (ValueError) and +-inf/NaN
        # trip the stiffness estimate (RuntimeError).
        with pytest.raises((ValueError, RuntimeError)):
            SpatialBackend().apply_transport(state, 0.5)
    ...
    def test_zero_interface_conductance_is_an_opaque_interface(self) -> None:
        ...
        out = SpatialBackend().apply_transport(state, 0.5)
    ...
    def test_varying_gap_conserves_weighted_density(self) -> None:
        ...
        N1 = SpatialBackend()._n1_per_cell(state)
    ...
    def test_interface_differs_from_bulk(self) -> None:
        ...
        def step(conductance: float | None) -> np.ndarray:
            ...
            return SpatialBackend().apply_transport(state, 0.5).f
