...
A1 = (p, q) = (1, 2) from the rejected diagnostic A2 = (2, 2) and the
scalar-Boltzmann closures C = (0, -1) and B = (0, -2):

* :mod:`uniform_gap_packet` -- the energy-dependent effective diffusivity
  ``D_eff(E)/D_N`` traces ``N_1^{q-p}``: rising (A1), flat (A2), falling
  (C), steeply falling (B); ``n_qp`` conserved.
* :mod:`gap_gradient_drift` -- the DOS-gradient drift velocity
  ``v = D_N q N_1^{q-p-1} d_x N_1`` (A1/A2 drift up the gap, C/B down).
* :mod:`interface_trap` -- a Kaplan-Larkin two-gap interface: current
  continuity + ``f``-discontinuity, and A1-vs-A2 distinct closed equilibria.
...
#: The four operators the §7.5 benchmarks compare.
