...
def _fail(path: Path, message: str) -> ValueError:
    return ValueError(f"Fischer 2024 companion at {path} {message}.")


def _classic_xref(payload: bytes, *, path: Path) -> dict[int, tuple[int, int]]:
    """Parse the classic xref table emitted by Matplotlib."""
...
def _content_tokens(payload: bytes) -> list[bytes]:
    ...
    This is deliberately a small PDF content-stream lexer, not a general
    parser.  It is sufficient to distinguish executable operators from
    operator-like bytes embedded in labels such as ``(S)`` or names such as
    ``/Do``.  Inline images are accepted as soon as their ``BI`` operator is
    seen, so their binary payload never needs tokenization.
    """
    tokens: list[bytes] = []
    cursor = 0
    size = len(payload)
...
def _has_visible_semantic_mark(decoded_streams: list[bytes]) -> bool:
    """Return whether Matplotlib painted anything beyond its canvas.

    A blank Matplotlib figure still paints one full-page white rectangle,
    so merely requiring a nonempty content stream (or any ``f`` operator)
    accepts a visually blank PDF.  Matplotlib emits that canvas as its first
    pure fill.  A later fill is a real patch; strokes, text-show operators,
    images/XObjects, and shadings are independently visible marks.
    """
...
def validate_single_nonempty_matplotlib_pdf(
    ...
    *,
    ...
    if startxref is None:  # already rejected by _classic_xref
        raise _fail(path, "has no terminal PDF xref/EOF record")
