...
> recorded single-thread environment. The checked score is `360646f2…2629`;
...
> its CSV/PDF/promotion record are promoted at `8a833204…42ac` /
> `52f7372c…0837` / `af620406…58bd`. Fig. 5's six single-thread continuation
...
| 3 | T3 reproduction regression against self-pinned baselines | Round-8 current-source F23 Figs. 3/5/6/7 plus hardened F24/M25 and transient artifacts are complete. Figs. 9--13 remain quarantined. None of these self-pins establishes paper parity. |
| 4 | Full Layer-4 audit chain | ✅ Current local tree: default **2509 passed, 1 skipped, 20 deselected, 12 warnings in 836.01 s**; bounded non-manual slow **15 passed, 1 expected xfail, 2514 deselected, 1 warning in 4497.19 s**; opt-in Fig. 5 row reauthentication **1 passed in 96.14 s**. Hosted CI remains separate post-push evidence. |
...
figure table below.
M25 baseline comparisons run on every platform: `rtol=1e-6` on the recorded
producer platform and a conservative `rtol=1e-3` fallback elsewhere for final
rounding differences. The post-normalization branch is unique; the wider
cross-platform envelope is not permission for branch jumps. Current M25
tables/PDFs are published as OS-locked, manifest-authenticated bundles.
...
Ph0 state qualified below. Fig. 5 v3 and Fig. 6 v2 persist complete `f/n_ph`
...
| Marchegiani 2025 Fig 3 (μ_α vs T) | `validation/marchegiani_2025/fig3_chemical_potentials.py` | `marchegiani_2025/` | ✅ OS-locked, manifest-authenticated qpsim regression with persisted branch states, reader-reassembled full residuals, and transcribed paper-formula μ inversion (SI Eqs. S2–S5). Broad topology/manual anchors are checked; no digitized pointwise paper-data comparison exists. |
| Marchegiani 2025 Fig 3 paper-styled panels | `validation/marchegiani_2025/fig3_paper.py` | `marchegiani_2025/` | ✅ authenticated paper-topology qpsim bundle with insets/regime shading and reader-reassembled state residuals; strict producer-platform pin `rtol=1e-6`, conservative cross-platform `rtol=1e-3`; no digitized parity. |
| Marchegiani 2025 Fig 4 (Γ_P, Γ̃^eo_01/Γ̃^eo_10 vs T) | `validation/marchegiani_2025/fig4_parity_rates.py` | `marchegiani_2025/` | ✅ manifest-authenticated summary-observable qpsim regression. Raw branch state is omitted, so certificate metadata is a producer assertion; broad trends/manual anchors only. |
| Marchegiani 2025 Fig 4 paper-styled two-stack | `validation/marchegiani_2025/fig4_paper.py` | `marchegiani_2025/` | ✅ authenticated summary-only paper-topology bundle: full, global-QE, renormalized, and detailed-balance comparison curves. Producer assertions are explicit; no reader-side state reconstruction or digitized parity. |
