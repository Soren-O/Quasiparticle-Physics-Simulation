...
| Capability | Status |
|---|---|
| Energy- and space-resolved kinetics (diffusion and spatial backends): fixed-gap ETD2, persistent-coordinate stage-constrained moving-gap ETD2, coupled Newton | ✅ moving-gap order verified against a refined reference within its documented DAE/support domain |
| Local phonon bath with acoustic escape | ✅ model decisions D1–D5 resolved in `docs/Phonon_Model_Decisions.md` |
...
uniform gap). Optional `gap_per_cell`
