...
| 8 | Marchegiani junction path (strategy A: rate equation) | ✅ Eq. 8 Lambert-W T̄ + coefficients-in steady-state solver + SI Notes III/IV coefficient integrals + SI Note V photon-assisted spectral density (`M25PhotonDrive` dataclass + back-solver) + population-dependent `g_ph_α_per_state` arrays all shipped. **M25 Fig 3a reproduced via the Layer-2 Device Architecture path** (see row below): the plotted quantity μ_L/Δ_L matches paper to ~0.3% at T = 20 mK; underlying moment quantities (x_L etc.) sit on a flat-valley multi-stable manifold where many seeds give the same plotted μ_L. Direct path: `solve_rate_equation_steady_state_multi_seed` (post-Phase-6 deterministic helper); the lower-level `solve_rate_equation_steady_state` accepts a single seed and may need `accept_lm_convergence=True` for the cancellation-floor "no progress" stall (the bypass is gated to that specific status + a 1.0 Hz residual ceiling, not arbitrary failures) |
| Device Architecture | Region/Junction/Device/Qubit composition layer | ✅ Phases 1-6 shipped: design doc, ExternalFlux through T3 stack (Phase 2), Region+Junction+Device+solve_device_steady_state (Phase 3), Qubit+parity+JunctionQubitCoupling (Phase 4), `M25GapAsymmetricJJ` Junction subclass (Phase 5 v1), no-double-counting plumbing (Phase 5b: `owns_region_dissipation` flag, `external_dissipation_only=True` backend path, Device-solver routing), moment-solver wiring (Phase 5c: caches the 4-unknown moment-solver fixed point on first evaluate, sidesteps the cross-tunneling bootstrap problem), Fig 3 + Fig 4 paper-figure baselines under `validation/marchegiani_2025/` (Phase 6), and a switch from non-deterministic `scipy.optimize.root(method='lm')` to deterministic `method='hybr')` plus a `solve_rate_equation_steady_state_multi_seed` helper for paper-matching branch selection (Phase 6 hardening). **M25 Fig 3a comparison** (deterministic hybr / multi-seed branch picker): the plotted quantity in M25 Fig 3a is μ_α/Δ_L vs T. At T = 20 mK we get μ_L/Δ_L = 0.8944 (vs paper ≈0.8975, **0.3% agreement**), tracking the same approximately linear T-dependence toward zero at T̄ ≈ 150 mK in the large-asymmetry panel. The underlying moment quantities (x_L = 3.58e-6, x_R> = 1.45e-6, x_R< = 4.90e-8, p_1 = 3.97e-4) sit on a flat valley of stalled hybr fixed points — multiple seeds give different (x_L, x_R>, p_1) combinations that produce the same plotted μ_L within visual resolution. The seventh-session bypass narrowing did not change the Fig 3a baseline (both pre- and post-narrowing CSVs record x_L = 3.58e-6 at T = 20 mK); the previously-cited "~5% on x_L" figure compared a standalone-call result against paper, while the baseline is generated via the continuation-seeded panel sweep. **Open investigation:** plugging paper-exact moment values into the residual gives ‖R‖_∞ ≈ 119 Hz (not at the ~1e-12 float64 cancellation floor), pointing at a coefficient-level disagreement somewhere in the SI Notes III/IV/V evaluators — does not affect the plotted μ_α agreement but is worth follow-up. Ancillary fixes: `SpectralContext.active_mask` epsilon switched from `mean(dE)` to the bin spacing of the first bin above the gap (uniform-grid behavior unchanged; correctly handles piecewise grids) |
