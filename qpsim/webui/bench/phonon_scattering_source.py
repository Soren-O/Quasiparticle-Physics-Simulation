...
register(
    Benchmark(
        ...
        caveat=(
            ...
            "2. THIS ROUTE USES THE QUASIPARTICLE-SIDE KERNEL IN THE PHONON "
            "EQUATION, and the schema switch that says otherwise is inert here. "
            "SpatialCollisions.advance_phonons calls compute_phonon_source_sink "
            "with the raw K_s0 = (E_i-E_j)^2 K-/(tau_0 (k_B T_c)^3) and never "
            "passes K_s0_phonon_side, so the F&C 2023 Eq. 12 prefactor "
            "2K-/(pi Delta tau_0^PB) that the 0-D transient and steady-state "
            "routes use is NOT what runs on spatial_2d — and "
            "phonons.use_phonon_side_kernel (default True) is not forwarded by "
            "run_spatial_2d at all. This benchmark therefore says nothing about "
            "tau_0_pb_ns or about the paper-faithful phonon-side prefactor; it "
            "certifies the kernel this route actually uses. The two routes "
            "disagree about the physics of the same term, which is a finding, "
            "not a caveat — it is reported here because a one-file benchmark "
            "cannot fix it.\n"
